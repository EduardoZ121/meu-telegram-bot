...
def replicate_nsfw_edit(prompt, photo_bytes):
    """Edição NSFW usando Flux Kontext Pro — preserva identidade, sem filtros de conteúdo.
    Modelo: black-forest-labs/flux-kontext-pro (safety_tolerance=6).
    Retorna bytes PNG."""
    ...
    try:
        output = replicate.run("black-forest-labs/flux-kontext-pro", input=inp)
        url = None
        if isinstance(output, list) and output:
            url = output[0]
        elif hasattr(output, "url"):
            ...
            url = output
        if not url:
            raise RuntimeError("Kontext Pro sem output")
        url = str(url)
