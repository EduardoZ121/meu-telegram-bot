...
def category_templates_kb(cat_key, lang, page=0, per_page=8):
    data = get_poster_data()
    cat = data.get(cat_key)
    if not cat:
        return None
...
# ---------- IMAGE GENERATION ----------
# Nome user-facing do modelo (em vez de gpt-image-1)
...
def openai_generate_poster(prompt, photo_bytes=None, size=None, _attempt=1):
...
def callback_poster(call):
    ...
    if data.startswith("poster_cat_"):
        ...
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                                   reply_markup=category_templates_kb(cat_key, lang, 0),
                                   parse_mode='HTML')
        except Exception:
    ...
    if data.startswith("poster_pg_"):
        ...
        try:
            ...
            page = 0
        bot.answer_callback_query(call.id)
        kb = category_templates_kb(cat_key, lang, page)
        if kb:
            try:
