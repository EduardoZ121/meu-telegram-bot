...
def gerar_imagem_padrao(prompt, aspect_ratio="1:1", num_outputs=1, image_input=None, user_id=None, lang="pt"):
    ...
    try:
        ...
        if user_id and is_nsfw_attempt(prompt):
            notify_admin_nsfw(user_id, prompt, model="padrao->kontext-max", severity="auto-route")
            # Se tiver foto, redireciona para Kontext Max (NSFW)
            if image_input:
                # image_input pode ser data URL — extrair bytes
                photo_bytes = _data_url_to_bytes(image_input)
                if photo_bytes:
                    img_bytes = replicate_nsfw_edit(prompt, photo_bytes)
...
def openai_generate_poster(prompt, photo_bytes=None, size=None, _attempt=1, user_id=None, lang="pt", allow_nsfw_lite=False):
    ...
    # NSFW attempt detection + admin notify + AUTO-ROUTE para Kontext Max se for NSFW e tiver foto
    nsfw_flag = is_nsfw_attempt(prompt) or (allow_nsfw_lite and is_nsfw_attempt(use_prompt))
    if nsfw_flag and photo_bytes:
        # Skip gpt-image-1 (que rejeita) e vai direto para Kontext Max (NSFW)
        if user_id:
            notify_admin_nsfw(user_id, prompt, model="gpt-image-1->kontext-max(auto)", severity="auto-route")
        try:
            return replicate_nsfw_edit(prompt, photo_bytes)
        except Exception as e:
            logger.error(f"Kontext Max auto-route falhou: {e}")
