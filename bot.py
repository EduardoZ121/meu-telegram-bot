...
def show_posters_main_menu(chat_id, user_id, lang):
    ...
    if not data:
        ...
        return
    text = TP(lang, "menu_header", credits=creditos, cost=POSTER_COST)
...
def callback_poster(call):
    ...
    if data.startswith("poster_tpl_"):
        ...
        # Foto obrigatoria? (Music/Food)
        if cat["photo_required"] and not photo_bytes:
            user_states[user_id] = "poster_awaiting_photo"
            poster_states[user_id]["pending_template"] = (cat_key, tpl_id)
            bot.send_message(call.message.chat.id, TP(lang, "photo_required_warning"), parse_mode='HTML')
