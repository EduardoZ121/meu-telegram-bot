...
def mode_keyboard(lang="pt"):
    """Teclado de seleccao de modo (fast / advanced)."""
...
def cmd_modo(message):
    """Comando para trocar/verificar o modo (fast/advanced)."""
    user_id = message.from_user.id
    lang = get_user_lang(user_id)
...
def callback_mode_set(call):
    ...
    confirm_key = "mode_set_fast" if mode == "fast" else "mode_set_advanced"
    bot.send_message(call.message.chat.id, T(lang, confirm_key), parse_mode='HTML')
    time.sleep(0.4)
    show_main_menu(call.message.chat.id, user_id, lang)
