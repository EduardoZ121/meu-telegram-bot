...
def replicate_nsfw_edit(prompt, photo_bytes):
    """Edição NSFW usando Flux Kontext Max — preserva rosto/identidade, muda apenas a roupa.
    Modelo: black-forest-labs/flux-kontext-max (safety_tolerance=6).
    Retorna bytes PNG."""
    import replicate, requests as _req, io as _io
    if not photo_bytes:
        raise RuntimeError("NSFW edit requer foto de referencia.")
    # Prompt structure: muda SÓ a roupa, preserva tudo o resto
    final_prompt = (
        "Keep the EXACT same face, hair color, hairstyle, skin tone, body shape, pose, "
        "background, and lighting. Only change the clothing. "
        f"New clothing: {(prompt or '').strip()}"
    )
    inp = {
        "prompt": final_prompt[:2000],
        "input_image": _io.BytesIO(photo_bytes),
        "safety_tolerance": 6,
        "output_format": "png",
        "output_quality": 90,
    }
    try:
        output = replicate.run("black-forest-labs/flux-kontext-max", input=inp)
        url = None
        if isinstance(output, list) and output:
            url = output[0]
        elif hasattr(output, "url"):
            ...
            url = output
        if not url:
            raise RuntimeError("Kontext Max sem output")
        url = str(url)
        r = _req.get(url, timeout=120)
        r.raise_for_status()
        return r.content
    except Exception as e:
        logger.error(f"replicate_nsfw_edit err: {e}")
        raise
