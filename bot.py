...
client = OpenAI(api_key=(OPENAI_API_KEY or "dummy").strip(), timeout=180.0, max_retries=1)
# Cliente especifico para gpt-image-1 (que demora 60-120s e estoura o timeout default).
# Timeout 240s + 1 retry automatico do SDK.
client_img = OpenAI(api_key=(OPENAI_API_KEY or "dummy").strip(), timeout=240.0, max_retries=1)
...
USER_MODES_FILE = "user_modes.json"  # NOVO: Modos do bot (fast/advanced)
USERS_EVER_SEEN_FILE = "users_ever_seen.json"  # NOVO: registo permanente de quem ja recebeu creditos gratis
...
SYSLOGS_LOCK = Lock()
SYSCFG_LOCK = Lock()
MODES_LOCK = Lock()  # NOVO: lock para modos (fast/advanced)
EVER_SEEN_LOCK = Lock()  # NOVO: lock para registo de users ever-seen
BACKUP_LOCK = Lock()  # NOVO: lock para auto-backup Telegram
...
# ==================== MODOS DO BOT (fast / advanced) ====================
# fast      = "Edição Rápida" → fluxo simplificado: foto + 65+ estilos prontos (1 clique)
# advanced  = "Modo Avançado" → bot completo (atual, com Pro, Carrossel, Wizard, Vídeo, Chat IA, etc.)

VALID_MODES = ("fast", "advanced", "posters")

def get_user_mode(user_id):
    """Retorna o modo escolhido pelo utilizador. None se ainda nao escolheu."""
    data = load_json(USER_MODES_FILE)
...
def set_user_mode(user_id, mode):
    ...
    save_json(USER_MODES_FILE, data, MODES_LOCK)
    logger.info(f"User {user_id} mode -> {mode}")
    return True

def has_chosen_mode(user_id):
    return get_user_mode(user_id) is not None

# ==================== USERS EVER SEEN (anti-abuso de creditos gratis) ====================
# Registo permanente: qualquer user que JA recebeu creditos gratis fica aqui para sempre.
# Mesmo se o ficheiro user_credits.json for apagado (ex: em redeploy do Render),
# o ficheiro users_ever_seen.json e um set/list que cresce so → mais resiliente.
# Em conjunto com o auto-backup para o Telegram do admin, garante zero abuso.

def is_user_ever_seen(user_id):
    """True se este user_id ja recebeu creditos gratis em qualquer momento."""
    data = load_json(USERS_EVER_SEEN_FILE)
    if isinstance(data, list):
        return str(user_id) in [str(x) for x in data]
    if isinstance(data, dict):
        return str(user_id) in data
    return False

def mark_user_ever_seen(user_id):
    """Marca este user_id como 'ja recebeu creditos gratis'. Idempotente."""
    data = load_json(USERS_EVER_SEEN_FILE)
    # Normalizar para dict {user_id: ISO_timestamp}
    if isinstance(data, list):
        data = {str(x): datetime.now().isoformat() for x in data}
    elif not isinstance(data, dict):
        data = {}
    uid = str(user_id)
    if uid not in data:
        data[uid] = datetime.now().isoformat()
        save_json(USERS_EVER_SEEN_FILE, data, EVER_SEEN_LOCK)
    return data

def migrate_existing_users_to_ever_seen():
    """Na 1a vez que este codigo corre, marca TODOS os users que ja existem
    no user_credits.json como ever_seen. Idempotente — pode correr varias vezes."""
    try:
        credits = load_json(CREDITS_FILE)
        ever = load_json(USERS_EVER_SEEN_FILE)
        if isinstance(ever, list):
            ever = {str(x): datetime.now().isoformat() for x in ever}
        elif not isinstance(ever, dict):
            ever = {}
        added = 0
        now_iso = datetime.now().isoformat()
        for uid in credits.keys():
            if uid not in ever:
                ever[uid] = now_iso
                added += 1
        if added > 0:
            save_json(USERS_EVER_SEEN_FILE, ever, EVER_SEEN_LOCK)
            logger.info(f"Migracao ever_seen: +{added} users marcados (total {len(ever)})")
    except Exception as e:
        logger.warning(f"Falha na migracao ever_seen: {e}")

# ==================== AUTO-BACKUP TELEGRAM (resistir a redeploy) ====================
# Cada vez que algo crítico muda (novo user, novo modo, novo idioma), o bot
# envia os ficheiros JSON ao admin (DM). Em redeploy, o admin reenvia para o bot
# o último backup e o /restore reescreve os ficheiros.
# Isto é um workaround para o filesystem efémero do Render — sem custo extra.

# Ficheiros considerados criticos para backup
CRITICAL_FILES = [
    "user_credits.json",
    "user_modes.json",
    "user_languages.json",
    "user_onboarding.json",
    "users_ever_seen.json",
    "user_settings.json",
    "user_history.json",
    "user_favorites.json",
    "referrals.json",
    "secondary_admins.json",
]

# Throttle: nao mandar mais que 1 backup a cada N segundos
_LAST_BACKUP_TS = {"ts": 0}
_BACKUP_THROTTLE_S = 60  # 1 minuto

def schedule_telegram_backup(reason="auto"):
    """Pede um backup async (em thread) com throttle anti-spam."""
    now = time.time()
    if now - _LAST_BACKUP_TS["ts"] < _BACKUP_THROTTLE_S:
        return  # ja foi enviado ha pouco
    _LAST_BACKUP_TS["ts"] = now
    Thread(target=_send_telegram_backup, args=(reason,), daemon=True).start()

def _send_telegram_backup(reason="auto"):
    """Envia ficheiros criticos como documents para SUPER_ADMIN_IDS[0] (DM)."""
    if not SUPER_ADMIN_IDS:
        return
    target = SUPER_ADMIN_IDS[0]
    sent = 0
    with BACKUP_LOCK:
        for fname in CRITICAL_FILES:
            path = fname  # ficheiros estao em /app/
            if not os.path.exists(path):
                continue
            try:
                # tag no caption para o admin saber que e backup
                caption = f"🗄️ backup · {fname} · {reason} · {datetime.now().strftime('%d/%m %H:%M')}"
                with open(path, "rb") as f:
                    bot.send_document(target, f, caption=caption[:1000])
                sent += 1
                time.sleep(0.4)  # rate limit telegram
            except Exception as e:
                logger.warning(f"Falha backup {fname}: {e}")
        if sent > 0:
            logger.info(f"Auto-backup enviado para admin: {sent} ficheiros ({reason})")

# ==================== I18N CENTRAL (mode picker, menus, common) ====================
# Tudo que o utilizador final ve passa por este dicionario.
# Adicionar uma chave nova? -> coloca aqui em PT/EN/ES/FR e usa T(lang, "key")

I18N = {
    # ---------- ONBOARDING / TERMS ----------
    "welcome_lang": {
        "pt": "🌍 <b>Bem-vindo!</b> / <b>Welcome!</b>\n\n🇵🇹 Escolhe o teu idioma:\n🇬🇧 Choose your language:\n🇪🇸 Elige tu idioma:\n🇫🇷 Choisis ta langue:",
        "en": "🌍 <b>Welcome!</b> / <b>Bem-vindo!</b>\n\n🇬🇧 Choose your language:\n🇵🇹 Escolhe o teu idioma:\n🇪🇸 Elige tu idioma:\n🇫🇷 Choisis ta langue:",
        "es": "🌍 <b>¡Bienvenido!</b> / <b>Welcome!</b>\n\n🇪🇸 Elige tu idioma:\n🇬🇧 Choose your language:\n🇵🇹 Escolhe o teu idioma:\n🇫🇷 Choisis ta langue:",
        "fr": "🌍 <b>Bienvenue!</b> / <b>Welcome!</b>\n\n🇫🇷 Choisis ta langue:\n🇬🇧 Choose your language:\n🇵🇹 Escolhe o teu idioma:\n🇪🇸 Elige tu idioma:",
    },
    "use_buttons_below": {
        "pt": "👇 Usa os botões abaixo para acederes ao menu:",
        "en": "👇 Use the buttons below to access the menu:",
        "es": "👇 Usa los botones de abajo para acceder al menú:",
        "fr": "👇 Utilise les boutons ci-dessous pour accéder au menu:",
    },
    "onboarding_q": {
        "pt": "🤖 <b>Bem-vindo ao Remake Pixel!</b> 👑\n\nJá usaste este bot antes?",
        "en": "🤖 <b>Welcome to Remake Pixel!</b> 👑\n\nHave you used this bot before?",
        "es": "🤖 <b>¡Bienvenido a Remake Pixel!</b> 👑\n\n¿Has usado este bot antes?",
        "fr": "🤖 <b>Bienvenue sur Remake Pixel!</b> 👑\n\nAs-tu déjà utilisé ce bot?",
    },
    "onboarding_yes": {
        "pt": "✅ Sim, já usei", "en": "✅ Yes, I've used it",
        "es": "✅ Sí, ya lo usé", "fr": "✅ Oui, déjà utilisé",
    },
    "onboarding_no": {
        "pt": "❌ Não, primeira vez", "en": "❌ No, first time",
        "es": "❌ No, primera vez", "fr": "❌ Non, première fois",
    },
    "tutorial_intro": {
        "pt": "🎨 <b>Bem-vindo ao Remake Pixel!</b>\n\nCria e edita imagens com IA — direto no Telegram.\n<b>50 créditos grátis</b> para experimentares!",
        "en": "🎨 <b>Welcome to Remake Pixel!</b>\n\nGenerate and edit images with AI — right inside Telegram.\n<b>50 free credits</b> to try it out!",
        "es": "🎨 <b>¡Bienvenido a Remake Pixel!</b>\n\nCrea y edita imágenes con IA — directo en Telegram.\n<b>50 créditos gratis</b> para probar!",
        "fr": "🎨 <b>Bienvenue sur Remake Pixel!</b>\n\nGénère et édite des images avec l'IA — directement dans Telegram.\n<b>50 crédits gratuits</b> pour tester!",
    },
    "terms_title": {
        ...
        "en": "📋 <b>Terms of Use</b>\n\n• Images for personal and creative use\n• User responsible for the content\n• Credits non-refundable\n• Service may be modified",
        "es": "📋 <b>Términos de Uso</b>\n\n• Imágenes para uso personal y creativo\n• Usuario responsable del contenido\n• Créditos no reembolsables\n• El servicio puede ser modificado",
        "fr": "📋 <b>Conditions d'utilisation</b>\n\n• Images pour usage personnel et créatif\n• Utilisateur responsable du contenu\n• Crédits non remboursables\n• Le service peut être modifié",
    },
    "terms_accept": {"pt": "✅ Aceito", "en": "✅ Accept", "es": "✅ Acepto", "fr": "✅ J'accepte"},
    "terms_decline": {"pt": "❌ Recuso", "en": "❌ Decline", "es": "❌ Rechazo", "fr": "❌ Refuser"},
    "terms_declined": {
        "pt": "❌ Termos recusados. Envia /start para tentares de novo.",
        "en": "❌ Terms declined. Send /start to try again.",
        "es": "❌ Términos rechazados. Envía /start para intentarlo de nuevo.",
        "fr": "❌ Conditions refusées. Envoie /start pour réessayer.",
    },

    # ---------- MODE PICKER ----------
    "mode_picker_title": {
        "pt": ("👇 <b>Escolhe o teu modo de uso</b>\n\n"
               "⚡ <b>Edição Rápida</b>\n"
               "Para quem quer resultado rápido e sem complicações.\n"
               "→ Envias uma foto e escolhes entre <b>65+ estilos prontos</b> (1 clique = resultado).\n\n"
               "🧠 <b>Modo Avançado</b>\n"
               "Para quem quer controlo total.\n"
               "→ Modelo Padrão, Pro, Artístico, Carrossel, Vídeo, Chat IA, Wizard, Estilos múltiplos, Configurações finas.\n\n"
               "🎨 <b>Posters Pro</b>\n"
               "Templates profissionais prontos (Música, Comida, Fitness, Motivacional, Flyers).\n"
               "→ 44 designs fixos · IA preenche os textos · gpt-image-1 OpenAI · 15 cr/imagem.\n\n"
               "<i>Podes trocar a qualquer momento em ⚙️ Configurações ou /modo.</i>"),
        "en": ("👇 <b>Choose your mode</b>\n\n"
               "⚡ <b>Quick Edit</b>\n"
               "For users who want fast results, no fuss.\n"
               "→ Send a photo and pick from <b>65+ ready styles</b> (1 click = result).\n\n"
               "🧠 <b>Advanced Mode</b>\n"
               "For users who want full control.\n"
               "→ Standard, Pro, Artistic models, Carousel, Video, AI Chat, Wizard, multi-style, fine settings.\n\n"
               "🎨 <b>Posters Pro</b>\n"
               "Ready-made professional templates (Music, Food, Fitness, Motivational, Flyers).\n"
               "→ 44 fixed designs · AI fills the text · gpt-image-1 OpenAI · 15 cr/image.\n\n"
               "<i>You can switch anytime in ⚙️ Settings or with /mode.</i>"),
        "es": ("👇 <b>Elige tu modo de uso</b>\n\n"
               "⚡ <b>Edición Rápida</b>\n"
               "Para quien quiere resultados rápidos sin complicaciones.\n"
               "→ Envía una foto y elige entre <b>65+ estilos listos</b> (1 clic = resultado).\n\n"
               "🧠 <b>Modo Avanzado</b>\n"
               "Para quien quiere control total.\n"
               "→ Modelo Estándar, Pro, Artístico, Carrusel, Video, Chat IA, Wizard, multi-estilos, ajustes finos.\n\n"
               "🎨 <b>Posters Pro</b>\n"
               "Plantillas profesionales listas (Música, Comida, Fitness, Motivacional, Flyers).\n"
               "→ 44 diseños fijos · IA rellena los textos · gpt-image-1 OpenAI · 15 cr/imagen.\n\n"
               "<i>Puedes cambiar en ⚙️ Configuración o con /modo.</i>"),
        "fr": ("👇 <b>Choisis ton mode</b>\n\n"
               "⚡ <b>Édition Rapide</b>\n"
               "Pour ceux qui veulent un résultat rapide, sans complications.\n"
               "→ Envoie une photo et choisis parmi <b>65+ styles prêts</b> (1 clic = résultat).\n\n"
               "🧠 <b>Mode Avancé</b>\n"
               "Pour ceux qui veulent le contrôle total.\n"
               "→ Modèles Standard, Pro, Artistique, Carrousel, Vidéo, Chat IA, Assistant, multi-styles, réglages fins.\n\n"
               "🎨 <b>Posters Pro</b>\n"
               "Templates professionnels prêts (Musique, Nourriture, Fitness, Motivation, Flyers).\n"
               "→ 44 designs fixes · IA remplit le texte · gpt-image-1 OpenAI · 15 cr/image.\n\n"
               "<i>Tu peux changer à tout moment dans ⚙️ Réglages ou via /mode.</i>"),
    },
    "mode_btn_posters": {
        "pt": "🎨 Posters Pro", "en": "🎨 Posters Pro",
        "es": "🎨 Posters Pro", "fr": "🎨 Posters Pro",
    },
    "mode_btn_fast": {
        "pt": "⚡ Edição Rápida",
        "en": "⚡ Quick Edit",
    ...
    "mode_set_fast": {
        "pt": "⚡ <b>Modo Edição Rápida</b> ativado!\n\nEnvia-me uma <b>foto</b> e escolho-te os <b>65+ estilos prontos</b> num só clique. Sem complicações.",
        "en": "⚡ <b>Quick Edit mode</b> activated!\n\nJust send me a <b>photo</b> and pick one of the <b>65+ ready styles</b> in a single tap. Zero hassle.",
        "es": "⚡ <b>Modo Edición Rápida</b> activado!\n\nEnvíame una <b>foto</b> y elige uno de los <b>65+ estilos listos</b> con un solo clic. Sin complicaciones.",
        "fr": "⚡ <b>Mode Édition Rapide</b> activé!\n\nEnvoie-moi une <b>photo</b> et choisis l'un des <b>65+ styles prêts</b> en un clic. Sans prise de tête.",
    },
    "mode_set_advanced": {
        "pt": "🧠 <b>Modo Avançado</b> ativado!\n\nTens acesso a tudo: Padrão, Pro, Artístico, Carrossel, Vídeo, Chat IA, Wizard e Configurações.",
        "en": "🧠 <b>Advanced mode</b> activated!\n\nYou now have access to everything: Standard, Pro, Artistic, Carousel, Video, AI Chat, Wizard and Settings.",
        "es": "🧠 <b>Modo Avanzado</b> activado!\n\nTienes acceso a todo: Estándar, Pro, Artístico, Carrusel, Video, Chat IA, Wizard y Configuración.",
        "fr": "🧠 <b>Mode Avancé</b> activé!\n\nTu as accès à tout: Standard, Pro, Artistique, Carrousel, Vidéo, Chat IA, Assistant et Réglages.",
    },
    "switch_mode": {
        "pt": "🔄 Trocar Modo", "en": "🔄 Switch Mode",
        "es": "🔄 Cambiar Modo", "fr": "🔄 Changer de mode",
    },
    "current_mode_label": {
        "pt": "Modo atual", "en": "Current mode",
        "es": "Modo actual", "fr": "Mode actuel",
    },

    # ---------- MAIN MENU ----------
    "menu_header_advanced": {
        "pt": "🎨 <b>Remake Pixel</b>\n\n👥 {users} utilizadores | 💳 <code>{credits}</code> créditos\n🧠 {mode_label}: <b>Avançado</b>\n\nO que desejas fazer?",
        "en": "🎨 <b>Remake Pixel</b>\n\n👥 {users} users | 💳 <code>{credits}</code> credits\n🧠 {mode_label}: <b>Advanced</b>\n\nWhat would you like to do?",
        "es": "🎨 <b>Remake Pixel</b>\n\n👥 {users} usuarios | 💳 <code>{credits}</code> créditos\n🧠 {mode_label}: <b>Avanzado</b>\n\n¿Qué deseas hacer?",
        "fr": "🎨 <b>Remake Pixel</b>\n\n👥 {users} utilisateurs | 💳 <code>{credits}</code> crédits\n🧠 {mode_label}: <b>Avancé</b>\n\nQue souhaites-tu faire?",
    },
    "menu_header_fast": {
        "pt": ("⚡ <b>Remake Pixel · Edição Rápida</b>\n\n"
               "💳 Créditos: <code>{credits}</code>\n"
               "👇 Envia-me uma <b>foto</b> e escolhes entre <b>65+ estilos prontos</b>.\n\n"
               "Ou usa um botão abaixo:"),
        "en": ("⚡ <b>Remake Pixel · Quick Edit</b>\n\n"
               "💳 Credits: <code>{credits}</code>\n"
               "👇 Send a <b>photo</b> and pick one of <b>65+ ready styles</b>.\n\n"
               "Or use a button below:"),
        "es": ("⚡ <b>Remake Pixel · Edición Rápida</b>\n\n"
               "💳 Créditos: <code>{credits}</code>\n"
               "👇 Envía una <b>foto</b> y elige entre <b>65+ estilos listos</b>.\n\n"
               "O usa un botón abajo:"),
        "fr": ("⚡ <b>Remake Pixel · Édition Rapide</b>\n\n"
               "💳 Crédits: <code>{credits}</code>\n"
               "👇 Envoie une <b>photo</b> et choisis parmi <b>65+ styles prêts</b>.\n\n"
               "Ou utilise un bouton ci-dessous:"),
    },
    # Botoes do menu rapido (fast)
    "fast_btn_styles": {
        "pt": "🎨 Ver 65+ Estilos", "en": "🎨 View 65+ Styles",
        "es": "🎨 Ver 65+ Estilos", "fr": "🎨 Voir 65+ Styles",
    },
    "fast_btn_credits": {
        "pt": "💳 Créditos", "en": "💳 Credits", "es": "💳 Créditos", "fr": "💳 Crédits",
    },
    "fast_btn_buy": {
        "pt": "🛒 Comprar", "en": "🛒 Buy", "es": "🛒 Comprar", "fr": "🛒 Acheter",
    },
    "fast_btn_help": {
        "pt": "❓ Ajuda", "en": "❓ Help", "es": "❓ Ayuda", "fr": "❓ Aide",
    },
    "fast_btn_history": {
        "pt": "📚 Histórico", "en": "📚 History", "es": "📚 Historial", "fr": "📚 Historique",
    },
    "fast_btn_lang": {
        "pt": "🌍 Idioma", "en": "🌍 Language", "es": "🌍 Idioma", "fr": "🌍 Langue",
    },
    "fast_help": {
        "pt": ("⚡ <b>Modo Edição Rápida — Como funciona</b>\n\n"
               "1️⃣ Envia uma <b>foto</b> aqui no chat\n"
               "2️⃣ Eu mostro-te <b>65+ estilos prontos</b> (Homens, Mulheres, Unissex, Flyers, Casais, Comics)\n"
               "3️⃣ Tocas no estilo → recebes a foto editada\n\n"
               f"💰 Custo: <b>{{cost}} créditos</b> por edição\n"
               "🔄 Para opções avançadas (Pro, Carrossel, Vídeo, Chat IA…) toca em <b>🔄 Trocar Modo</b>."),
        "en": ("⚡ <b>Quick Edit Mode — How it works</b>\n\n"
               "1️⃣ Send a <b>photo</b> in the chat\n"
               "2️⃣ I show you <b>65+ ready styles</b> (Men, Women, Unisex, Flyers, Couples, Comics)\n"
               "3️⃣ Tap a style → get the edited photo back\n\n"
               f"💰 Cost: <b>{{cost}} credits</b> per edit\n"
               "🔄 For advanced features (Pro, Carousel, Video, AI Chat…) tap <b>🔄 Switch Mode</b>."),
        "es": ("⚡ <b>Modo Edición Rápida — Cómo funciona</b>\n\n"
               "1️⃣ Envía una <b>foto</b> al chat\n"
               "2️⃣ Te muestro <b>65+ estilos listos</b> (Hombres, Mujeres, Unisex, Flyers, Parejas, Comics)\n"
               "3️⃣ Tocas un estilo → recibes la foto editada\n\n"
               f"💰 Coste: <b>{{cost}} créditos</b> por edición\n"
               "🔄 Para opciones avanzadas (Pro, Carrusel, Video, Chat IA…) toca <b>🔄 Cambiar Modo</b>."),
        "fr": ("⚡ <b>Mode Édition Rapide — Comment ça marche</b>\n\n"
               "1️⃣ Envoie une <b>photo</b> dans le chat\n"
               "2️⃣ Je te montre <b>65+ styles prêts</b> (Hommes, Femmes, Unisexe, Flyers, Couples, Comics)\n"
               "3️⃣ Touche un style → reçois la photo éditée\n\n"
               f"💰 Coût: <b>{{cost}} crédits</b> par édition\n"
               "🔄 Pour les options avancées (Pro, Carrousel, Vidéo, Chat IA…) touche <b>🔄 Changer de mode</b>."),
    },

    # ---------- COMUNS ----------
    "back": {"pt": "◀️ Voltar", "en": "◀️ Back", "es": "◀️ Volver", "fr": "◀️ Retour"},
    "cancel": {"pt": "❌ Cancelar", "en": "❌ Cancel", "es": "❌ Cancelar", "fr": "❌ Annuler"},
    "send_start_first": {
        "pt": "👋 Envia /start para começar!",
    ...
    "must_pick_mode": {
        ...
        "en": "👇 Before continuing, choose your mode:",
        "es": "👇 Antes de continuar, elige tu modo:",
        "fr": "👇 Avant de continuer, choisis ton mode:",
    },
}

def T(lang, key, **fmt):
    """Helper de traducao com fallback PT."""
    entry = I18N.get(key, {})
...
def get_user_credits(user_id):
    ...
    if user_id_str not in data:
        # 🛡️ ANTI-ABUSO: Se este user_id JA recebeu creditos gratis no passado
        # (mesmo apos um redeploy que apagou o JSON local), NAO dar de novo.
        already_seen = False
        try:
            already_seen = is_user_ever_seen(user_id)
        except Exception:
            pass
        starting_credits = 0 if already_seen else 50
        data[user_id_str] = {
            "creditos": starting_credits,
            "total_usado": 0,
            "historico": [],
            "created_at": datetime.now().isoformat(),
            "ever_received_free": True
        }
        save_json(CREDITS_FILE, data, CREDITS_LOCK)
        try:
            mark_user_ever_seen(user_id)
        except Exception:
            pass
        if already_seen:
            logger.info(f"User {user_id} reapareceu (sem creditos gratis — ever_seen)")
        else:
            logger.info(f"Novo usuário {user_id} com 50 créditos")
            # Auto-backup — envia user_credits.json + users_ever_seen.json para o admin
            try:
                schedule_telegram_backup(reason=f"new_user_{user_id}")
            except Exception:
                pass
        ...
        try:
            for admin_id in ADMIN_IDS:
                bot.send_message(
                    admin_id,
                    f"🎉 <b>{'NOVO USUÁRIO' if not already_seen else 'USER REAPARECEU'}!</b>\n\n"
                    f"👤 ID: <code>{user_id}</code>\n"
                    f"💳 Créditos iniciais: <b>{starting_credits}</b>"
                    f"{' (já tinha recebido antes — bloqueado)' if already_seen else ''}\n"
...
def gerar_imagem_padrao(prompt, aspect_ratio="1:1", num_outputs=1, image_input=None, user_id=None, lang="pt"):
    """Gera imagem usando modelo padrao com aspect ratio configuravel.
    Aplica SAFETY REWRITE para preservar identidade (rosto, corpo, idade).
    Notifica admin em tentativas NSFW.
    AUTO-ROUTING: se prompt for NSFW (biquini/lingerie) e tiver foto -> envia DIRETO para FLUX
    em vez de usar Grok (que rejeita), preservando identidade. Notifica admin."""
    try:
        # 1. NSFW detection + admin notify + auto-routing para FLUX
        if user_id and is_nsfw_attempt(prompt):
            notify_admin_nsfw(user_id, prompt, model="padrao->flux", severity="auto-route")
            # Se tiver foto, redireciona para FLUX (que permite NSFW lite)
            if image_input:
                # image_input pode ser data URL — extrair bytes
                photo_bytes = _data_url_to_bytes(image_input)
                if photo_bytes:
                    safe = safety_rewrite_prompt(prompt, lang=lang,
                                                  has_reference_photo=True,
                                                  allow_nsfw_lite=True)
                    if safe is None:
                        raise RuntimeError("Pedido bloqueado: conteudo explicito.")
                    img_bytes = replicate_flux_edit(safe or prompt, photo_bytes)
                    # Caller espera lista de URLs — guarda como data URL para enviar
                    import base64 as _b64
                    return ["data:image/png;base64," + _b64.b64encode(img_bytes).decode()]
        # 2. Safety rewrite normal (sem NSFW lite)
        safe_prompt = safety_rewrite_prompt(prompt, lang=lang,
                                             has_reference_photo=bool(image_input),
                                             allow_nsfw_lite=False)
        if safe_prompt is None:
            raise RuntimeError("Pedido bloqueado: conteudo explicito.")
        input_params = {
            "prompt": safe_prompt or prompt,
...
# Alias retro-compativel: muito codigo legacy chama gerar_imagem_modelo
gerar_imagem_modelo = gerar_imagem_padrao


def _force_aspect_ratio(image_bytes, ratio_str):
    """Pós-processa bytes de uma imagem para forçar o aspect_ratio escolhido pelo user.
    Faz center-crop inteligente (ImageOps.fit) para evitar distorção. Devolve bytes JPEG.
    Aceita strings como '3:4', '1:1', '16:9', '9:16', '4:5', '21:9'."""
    try:
        if not ratio_str or ":" not in str(ratio_str):
            return image_bytes
        w_part, h_part = ratio_str.split(":")
        target_w = float(w_part)
        target_h = float(h_part)
        ...
        # Se já está praticamente certo (tolerância 1.5%), nem mexe
        if abs(cur_ratio - target_ratio) / target_ratio < 0.015:
            return image_bytes

        # Calcula tamanho final mantendo o lado maior possível sem upscale
        if target_ratio >= cur_ratio:
            # Imagem alvo é mais larga (ou igual) — limita pela largura atual
            new_w = cur_w
            new_h = int(round(cur_w / target_ratio))
        else:
            # Imagem alvo é mais alta — limita pela altura atual
            new_h = cur_h
            new_w = int(round(cur_h * target_ratio))

        # ImageOps.fit faz crop centralizado para o tamanho exato
        if img.mode == "RGBA":
            img = img.convert("RGB")
        out = ImageOps.fit(img, (new_w, new_h), method=Image.LANCZOS, centering=(0.5, 0.5))
        ...
        try:
            logger.warning(f"_force_aspect_ratio falhou: {_e}")
        except Exception:
            pass
        return image_bytes


def gerar_imagem_pro(image_input, prompt_override=None, user_id=None, lang="pt", allow_nsfw_lite=False):
    """Melhora imagem usando Modelo Pro (FLUX.2 Klein 9B).
    Se prompt_override for fornecido, usa-o em vez do prompt_fixo.
    Aplica SAFETY REWRITE + NSFW notify.
    AUTO-ROUTING: se NSFW detectado, ativa allow_nsfw_lite automaticamente."""
    try:
        prompt_raw = prompt_override if prompt_override else MODELO_PRO["prompt_fixo"]
        # Auto-elevar para NSFW lite se detectado (FLUX permite — Stripe-safe limit)
        if user_id and is_nsfw_attempt(prompt_raw):
            allow_nsfw_lite = True  # auto-ativa
            notify_admin_nsfw(user_id, prompt_raw, model="pro(flux)->lite", severity="auto-route")
        prompt_final = safety_rewrite_prompt(prompt_raw, lang=lang,
                                              has_reference_photo=True,
                                              allow_nsfw_lite=allow_nsfw_lite)
        if prompt_final is None:
            raise RuntimeError("Pedido bloqueado: conteudo explicito.")
        # Reforcar identity-anchor para garantir que FLUX nao troca a pessoa
        identity_anchor = (
            "Edit the EXACT person from the reference image. "
            "Keep face, hair, skin tone, age, body proportions and identity 100% identical. "
            "Do NOT replace with another person. "
        )
        input_params = {
            "prompt": identity_anchor + (prompt_final or prompt_raw),
            "images": [image_input],
            "aspect_ratio": "match_input_image",
            "safety_tolerance": 6,
            "disable_safety_checker": True
...
user_states = {}
pending_photos = {}
pending_padst_choice = {}  # user_id -> {"style_key": str, "style_nome": str} — aguarda escolha de formato após clicar num estilo do Modelo Padrão
...
def mode_keyboard(lang="pt"):
    """Teclado de seleccao de modo (fast / advanced / posters)."""
    markup = telebot.types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "mode_btn_fast"), callback_data="mode_set_fast"),
        telebot.types.InlineKeyboardButton(T(lang, "mode_btn_advanced"), callback_data="mode_set_advanced"),
        telebot.types.InlineKeyboardButton(T(lang, "mode_btn_posters"), callback_data="mode_set_posters"),
    )
    return markup

def show_mode_picker(chat_id, user_id, lang=None, edit_message_id=None):
    """Mostra o ecra de escolha de modo. Usado apos /start (apos termos) ou
    quando o utilizador toca em '🔄 Trocar Modo'."""
    if lang is None:
        lang = get_user_lang(user_id)
    txt = T(lang, "mode_picker_title")
    if edit_message_id:
        try:
            bot.edit_message_text(txt, chat_id, edit_message_id,
                                   reply_markup=mode_keyboard(lang), parse_mode='HTML')
            return
        except Exception:
            pass
    bot.send_message(chat_id, txt, reply_markup=mode_keyboard(lang), parse_mode='HTML')

def onboarding_keyboard(lang="pt"):
    markup = telebot.types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "onboarding_yes"), callback_data="onboard_yes"),
        telebot.types.InlineKeyboardButton(T(lang, "onboarding_no"), callback_data="onboard_no")
    )
    return markup
...
def main_keyboard(lang="pt"):
    ...
    # >>> Trocar de modo (fast/advanced) — sempre visível para o utilizador alternar
    markup.add(telebot.types.InlineKeyboardButton(T(lang, "switch_mode"), callback_data="action_switch_mode"))
    return markup
...
def settings_keyboard(lang="pt"):
    ...
    texts = {
        ...
        "fr": ["🎨 Styles", "📐 Format", "🔢 Variations", "🤖 Personnalité", "◀️ Retour"],
    }
    t = texts.get(lang, texts["pt"])
    markup.add(
    ...
    # Opcao para trocar modo (fast/advanced) e idioma
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "switch_mode"), callback_data="action_switch_mode"),
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_lang"), callback_data="action_change_lang"),
    )
...
# ==================== HELPERS ====================
def main_keyboard_fast(user_id, lang="pt"):
    """Menu simplificado para o modo Edicao Rapida."""
    markup = telebot.types.InlineKeyboardMarkup(row_width=2)
    # 1a linha: 65+ estilos em destaque (full width)
    markup.add(telebot.types.InlineKeyboardButton(T(lang, "fast_btn_styles"), callback_data="fast_show_styles"))
    # 2a linha: creditos / comprar
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_credits"), callback_data="action_credits"),
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_buy"), callback_data="action_buy"),
    )
    # 3a linha: historico / idioma
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_history"), callback_data="action_history"),
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_lang"), callback_data="action_change_lang"),
    )
    # 4a linha: trocar modo + ajuda
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "switch_mode"), callback_data="action_switch_mode"),
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_help"), callback_data="action_help_fast"),
    )
    return markup
...
def show_main_menu(chat_id, user_id, lang):
    ...
    # Se o user ainda nao escolheu modo (legacy users), forca-o a escolher antes do menu
    if mode is None:
        bot.send_message(chat_id, T(lang, "must_pick_mode"), parse_mode='HTML')
        show_mode_picker(chat_id, user_id, lang)
        return

    if mode == "fast":
        text = T(lang, "menu_header_fast", credits=creditos)
        bot.send_message(chat_id, text, reply_markup=main_keyboard_fast(user_id, lang), parse_mode='HTML')
        return

    if mode == "posters":
        # Modo Posters Pro — categorias + utilitarios (definido no modulo Posters Pro)
        try:
            show_posters_main_menu(chat_id, user_id, lang)
            return
        except NameError:
            # se o modulo posters ainda nao foi avaliado por algum motivo, cai para advanced
            pass

    # Modo Avancado (default historico)
    text = T(lang, "menu_header_advanced",
             users=total_users, credits=creditos, mode_label=T(lang, "current_mode_label"))
    bot.send_message(chat_id, text, reply_markup=main_keyboard(lang), parse_mode='HTML')
...
def callback_language(call):
    ...
    if set_user_lang(user_id, lang_code):
        ...
        # Se o user JA fez onboarding, voltar ao menu (com lingua nova)
        if is_onboarded(user_id) and has_chosen_mode(user_id):
            show_main_menu(call.message.chat.id, user_id, lang_code)
            return

        # Caso contrario, continuar onboarding em onboarding_q
        print(f"📤 Enviando onboarding em {lang_code}")
        bot.send_message(call.message.chat.id, T(lang_code, "onboarding_q"),
                          reply_markup=onboarding_keyboard(lang_code), parse_mode='HTML')
...
def callback_onboarding(call):
    ...
    # Se o user disse que e novo, mostrar uma intro curta antes dos termos
    if action != "yes":
        bot.send_message(call.message.chat.id, T(lang, "tutorial_intro"), parse_mode='HTML')
        time.sleep(1)
    ...
    markup = telebot.types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "terms_accept"), callback_data="terms_accept"),
        telebot.types.InlineKeyboardButton(T(lang, "terms_decline"), callback_data="terms_decline")
    )
    bot.send_message(call.message.chat.id, T(lang, "terms_title"),
...
def callback_actions(call):
    ...
    if action in ["menu", "cancel"]:
        ...
        # 🔄 Trocar Modo (fast/advanced) — a partir de qualquer menu
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        show_mode_picker(call.message.chat.id, user_id, lang)
        return

    elif action == "change_lang":
        # 🌍 Trocar idioma a partir do menu
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        bot.send_message(call.message.chat.id, T(lang, "welcome_lang"),
                         reply_markup=language_keyboard(), parse_mode='HTML')
        return

    elif action == "help_fast":
        # ❓ Ajuda especifica do modo Edicao Rapida
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        cost = MODELO_PADRAO['custo'] if 'MODELO_PADRAO' in globals() else 10
        text = T(lang, "fast_help", cost=cost)
        mk = telebot.types.InlineKeyboardMarkup(row_width=1)
        mk.add(telebot.types.InlineKeyboardButton(T(lang, "switch_mode"), callback_data="action_switch_mode"))
        mk.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="action_menu"))
        ...
        share_text = "Experimenta o Remake Pixel! Cria e edita imagens com IA no Telegram. 50 créditos grátis!"
        share_url = link
...
def cmd_start(message):
    ...
    bot.send_message(
        message.chat.id,
        T(get_user_lang(user_id), "welcome_lang"),
    ...
    bot.send_message(
        message.chat.id,
        T(get_user_lang(user_id), "use_buttons_below"),
        reply_markup=get_main_reply_keyboard(user_id)
    )

@bot.message_handler(commands=['idioma', 'lang', 'language'])
def cmd_idioma(message):
    bot.send_message(message.chat.id, T(get_user_lang(message.from_user.id), "welcome_lang"),
                      reply_markup=language_keyboard(), parse_mode='HTML')

@bot.message_handler(commands=['modo', 'mode'])
def cmd_modo(message):
    """Comando para trocar/verificar o modo (fast/advanced/posters)."""
    user_id = message.from_user.id
    lang = get_user_lang(user_id)
...
def cmd_private(message):
    ...
    txt = {
        "pt": ("🔥 <b>Edição Privada</b> (Modelo Pro / FLUX)\n\n"
               f"💳 {PRIVATE_EDIT_COST} créditos por imagem\n\n"
               "1️⃣ Envia uma foto\n"
               "2️⃣ Descreve a edição que queres (ex: <i>biquíni preto na praia</i>, <i>lingerie em quarto</i>)\n\n"
               "🛡️ <b>Regras automáticas:</b>\n"
               "• Mantenho o teu rosto, idade e proporções <b>iguais</b>\n"
               "• Permitido: biquíni, lingerie, swimwear\n"
               "• <b>Bloqueado:</b> nudez total, conteúdo sexual explícito\n"
               "• Toda tentativa fica registada (admin é notificado)"),
        "en": ("🔥 <b>Private Edit</b> (Pro Model / FLUX)\n\n"
               f"💳 {PRIVATE_EDIT_COST} credits per image\n\n"
               "1️⃣ Send a photo\n"
               "2️⃣ Describe the edit (e.g. <i>black bikini on the beach</i>, <i>lingerie in a bedroom</i>)\n\n"
               "🛡️ <b>Auto rules:</b>\n"
               "• Your face, age and proportions stay <b>identical</b>\n"
               "• Allowed: bikini, lingerie, swimwear\n"
               "• <b>Blocked:</b> full nudity, explicit sexual content\n"
               "• Every attempt is logged (admin is notified)"),
        "es": ("🔥 <b>Edición Privada</b> (Modelo Pro / FLUX)\n\n"
               f"💳 {PRIVATE_EDIT_COST} créditos por imagen\n\n"
               "1️⃣ Envía una foto\n"
               "2️⃣ Describe la edición (ej: <i>bikini negro en la playa</i>, <i>lencería en habitación</i>)\n\n"
               "🛡️ <b>Reglas automáticas:</b>\n"
               "• Tu rostro, edad y proporciones se mantienen <b>idénticos</b>\n"
               "• Permitido: bikini, lencería\n"
               "• <b>Bloqueado:</b> desnudez total, contenido explícito\n"
               "• Todo intento se registra (admin es notificado)"),
        "fr": ("🔥 <b>Édition Privée</b> (Modèle Pro / FLUX)\n\n"
               f"💳 {PRIVATE_EDIT_COST} crédits par image\n\n"
               "1️⃣ Envoie une photo\n"
               "2️⃣ Décris l'édition (ex: <i>bikini noir sur la plage</i>, <i>lingerie</i>)\n\n"
               "🛡️ <b>Règles auto:</b>\n"
               "• Ton visage et tes proportions restent <b>identiques</b>\n"
               "• Autorisé: bikini, lingerie\n"
               "• <b>Bloqué:</b> nudité, contenu explicite\n"
...
def _private_run(chat_id, user_id, lang, photo_bytes, user_prompt):
    """Executa a edicao privada com FLUX + safety lite + notify admin."""
    ...
    try:
        img_bytes = replicate_flux_edit(
            safety_rewrite_prompt(user_prompt, lang=lang, has_reference_photo=True, allow_nsfw_lite=True) or user_prompt,
            photo_bytes,
        )
        # admin notify SEMPRE em private edit
        notify_admin_nsfw(user_id, user_prompt, model="private(flux)", severity="lite")
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
...
def cmd_backup(message):
    """ADMIN: forca envio dos ficheiros JSON criticos como documents para o admin."""
    user_id = message.from_user.id
    if user_id not in ADMIN_IDS:
        return
    bot.reply_to(message, "🗄️ A enviar backup dos ficheiros JSON criticos...")
    # ignora throttle quando for manual
    _LAST_BACKUP_TS["ts"] = 0
    schedule_telegram_backup(reason="manual")
    bot.send_message(user_id, "✅ Pedido de backup enviado. Verifica os ficheiros recebidos no chat.")

@bot.message_handler(commands=['restore'])
def cmd_restore(message):
    """ADMIN: instrucoes para restaurar ficheiros apos redeploy."""
    user_id = message.from_user.id
    if user_id not in ADMIN_IDS:
        return
    txt = (
        "🔄 <b>Como restaurar apos redeploy:</b>\n\n"
        "1. Abre o ultimo backup que recebeste neste chat.\n"
        "2. <b>Reenvia o ficheiro</b> .json para mim aqui no chat.\n"
        "   (Pode ser 1 a 1 ou em sequencia.)\n"
        "3. Eu vou reconhecer cada ficheiro pelo nome e <b>sobrescrever</b> o que esta no servidor.\n\n"
        "⚠️ Aceito apenas ficheiros cujo nome esteja na lista de criticos:\n"
        + "\n".join(f"• <code>{f}</code>" for f in CRITICAL_FILES) +
        "\n\nUsa /backup para forcar um novo backup a qualquer momento."
    )
...
def admin_doc_restore(message):
    """ADMIN-only: ao receber um JSON com nome de ficheiro critico, sobrescreve."""
    user_id = message.from_user.id
    if user_id not in ADMIN_IDS:
        return
    ...
    if not doc or not doc.file_name:
        return
    fname = doc.file_name
    if fname not in CRITICAL_FILES:
        bot.reply_to(message, f"⚠️ Ignorado: <code>{fname}</code> nao esta na lista de ficheiros criticos.\nUsa /restore para ver a lista.", parse_mode='HTML')
        return
    try:
        ...
        with open(fname, "wb") as f:
            f.write(content)
        # invalidar caches em memoria se relevantes
        global _POSTER_DATA
        if fname == "poster_templates.json":
            _POSTER_DATA = None
        bot.reply_to(message, f"✅ <b>Ficheiro restaurado:</b> <code>{fname}</code> ({len(content)} bytes)", parse_mode='HTML')
        logger.info(f"Admin {user_id} restaurou {fname} ({len(content)}B)")
    except json.JSONDecodeError:
        bot.reply_to(message, f"❌ <code>{fname}</code> nao e JSON valido. Restauro abortado.", parse_mode='HTML')
    except Exception as e:
        bot.reply_to(message, f"❌ Erro a restaurar <code>{fname}</code>: <code>{str(e)[:120]}</code>", parse_mode='HTML')
...
def handle_photo(message):
    ...
    if not is_onboarded(user_id):
        bot.reply_to(message, T(lang, "send_start_first"))
        return
    ...
    if user_states.get(user_id) == "poster_awaiting_photo":
        try:
            file_info = bot.get_file(message.photo[-1].file_id)
            downloaded = bot.download_file(file_info.file_path)
            st = poster_states.get(user_id) or {}
            st["photo_bytes"] = downloaded
            poster_states[user_id] = st
            user_states.pop(user_id, None)
            bot.reply_to(message, TP(lang, "photo_status_yes"))
            # Se ja havia template pendente, gera direto
            pending = st.get("pending_template")
            if pending:
                cat_key, tpl_id = pending
                st.pop("pending_template", None)
                Thread(target=_poster_generate_first,
                        args=(message.chat.id, user_id, lang, cat_key, tpl_id)).start()
            else:
                # mostra menu para o user escolher template
                show_posters_main_menu(message.chat.id, user_id, lang)
        except Exception as e:
            bot.reply_to(message, f"❌ Erro a processar foto: {e}")
        return

    # >>> POSTERS PRO (genérico): se o user está em modo posters e envia uma foto
    # qualquer fora do contexto, GUARDA automaticamente e abre o menu de categorias.
    # Mesmo fluxo do modo Padrão: foto → escolhe estilo → gera.
    if get_user_mode(user_id) == "posters":
        try:
            file_info = bot.get_file(message.photo[-1].file_id)
            downloaded = bot.download_file(file_info.file_path)
            st = poster_states.get(user_id) or {}
            ...
            poster_states[user_id] = st
            ack = {
                "pt": "✅ Foto guardada! Agora escolhe um template:",
                "en": "✅ Photo saved! Now pick a template:",
                "es": "✅ ¡Foto guardada! Ahora elige una plantilla:",
                "fr": "✅ Photo enregistrée! Choisis maintenant un template:",
            }.get(lang, "✅ OK")
    ...
    # >>> EDIÇÃO RÁPIDA: salta direto ao picker de 65+ estilos (Modelo Padrão), sem menu de 3 modelos.
    if get_user_mode(user_id) == "fast":
        if caption and len(caption.strip()) >= 3:
            # Tem legenda → executa Padrão direto (mesma logica do callback_photo_model action='padrao' COM legenda)
            if get_user_credits(user_id) < MODELO_PADRAO["custo"]:
                insuf = {"pt": "❌ Créditos insuficientes.",
                         "en": "❌ Not enough credits.",
                         "es": "❌ Créditos insuficientes.",
                         "fr": "❌ Crédits insuffisants."}
                bot.reply_to(message, insuf.get(lang, insuf["pt"]))
                return
            photo_data = pending_photos.pop(user_id)
            Thread(target=execute_padrao,
                   args=(message.chat.id, user_id, lang, photo_data, caption.strip(), None)).start()
            return

        # Sem legenda → mostrar diretamente categorias de 65+ estilos
        title = {
            "pt": "🎨 <b>Escolher Estilo</b> (65+)\n\nEscolhe a categoria:",
            "en": "🎨 <b>Choose Style</b> (65+)\n\nPick a category:",
            "es": "🎨 <b>Elegir Estilo</b> (65+)\n\nElige la categoría:",
            "fr": "🎨 <b>Choisir un Style</b> (65+)\n\nChoisis la catégorie:",
        }
        try:
            bot.reply_to(message, title.get(lang, title["pt"]),
                         reply_markup=_padrao_categories_markup(lang), parse_mode='HTML')
        except Exception:
            # fallback se _padrao_categories_markup falhar por qualquer motivo
            bot.reply_to(message, title.get(lang, title["pt"]))
...
def callback_terms(call):
    ...
    if action == "accept":
        bot.send_message(call.message.chat.id, T(lang, "tutorial_intro"), parse_mode='HTML')
        time.sleep(1)
        # >>> Apos termos, mostrar SEMPRE escolha de modo (fast / advanced)
        show_mode_picker(call.message.chat.id, user_id, lang)
    else:
        bot.send_message(call.message.chat.id, T(lang, "terms_declined"))


@bot.callback_query_handler(func=lambda call: call.data.startswith('mode_set_'))
def callback_mode_set(call):
    """Callback dos 2 botoes do mode_picker."""
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    mode = call.data.replace('mode_set_', '')
    if mode not in VALID_MODES:
        bot.answer_callback_query(call.id, "Invalid mode")
        return
    ...
    if mode == "fast":
        bot.send_message(call.message.chat.id, T(lang, "mode_set_fast"), parse_mode='HTML')
    elif mode == "advanced":
        bot.send_message(call.message.chat.id, T(lang, "mode_set_advanced"), parse_mode='HTML')
    else:  # posters
        try:
            bot.send_message(call.message.chat.id, TP(lang, "mode_set_posters"), parse_mode='HTML')
        except NameError:
            bot.send_message(call.message.chat.id, "🎨 Posters Pro mode activated!", parse_mode='HTML')
    time.sleep(0.4)
    show_main_menu(call.message.chat.id, user_id, lang)


@bot.callback_query_handler(func=lambda call: call.data == "action_switch_mode")
def callback_action_switch_mode(call):
    """Botao 'Trocar Modo' a partir do menu (qualquer modo)."""
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    bot.answer_callback_query(call.id)
    try:
...
@bot.callback_query_handler(func=lambda call: call.data == "action_change_lang")
def callback_action_change_lang(call):
    """Botao para trocar idioma a partir do menu rapido."""
    user_id = call.from_user.id
    bot.answer_callback_query(call.id)
    try:
        bot.delete_message(call.message.chat.id, call.message.message_id)
        ...
        pass
    bot.send_message(call.message.chat.id, T(get_user_lang(user_id), "welcome_lang"),
                     reply_markup=language_keyboard(), parse_mode='HTML')


@bot.callback_query_handler(func=lambda call: call.data == "action_help_fast")
def callback_action_help_fast(call):
    """Ajuda especifica do modo Edicao Rapida."""
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    bot.answer_callback_query(call.id)
...
@bot.callback_query_handler(func=lambda call: call.data == "fast_show_styles")
def callback_fast_show_styles(call):
    """Ao tocar em '🎨 Ver 65+ Estilos' no menu rapido."""
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    bot.answer_callback_query(call.id)
    msgs = {
        "pt": ("🎨 <b>65+ Estilos prontos</b>\n\n"
               "Para usares: <b>envia uma foto</b> aqui no chat e eu mostro-te o catálogo de estilos para tocares.\n\n"
               "💡 Sem foto, podes ver alguns exemplos com /estilos."),
        "en": ("🎨 <b>65+ Ready styles</b>\n\n"
               "To use them: <b>send a photo</b> in the chat and I'll show you the full style catalog to tap.\n\n"
               "💡 Without a photo, you can preview some with /estilos."),
        "es": ("🎨 <b>65+ Estilos listos</b>\n\n"
               "Para usarlos: <b>envía una foto</b> al chat y te muestro el catálogo de estilos para tocar.\n\n"
               "💡 Sin foto, puedes ver algunos con /estilos."),
        "fr": ("🎨 <b>65+ Styles prêts</b>\n\n"
               "Pour les utiliser: <b>envoie une photo</b> dans le chat et je te montre le catalogue de styles.\n\n"
               "💡 Sans photo, tu peux en voir certains avec /estilos."),
    }
    mk = telebot.types.InlineKeyboardMarkup(row_width=1)
    mk.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="action_menu"))
    bot.send_message(call.message.chat.id, msgs.get(lang, msgs["pt"]), reply_markup=mk, parse_mode='HTML')
...
def execute_padrao(chat_id, user_id, lang, photo_data, prompt_text, style_nome=None, style_key=None):
    ...
    try:
        ...
        for url in urls:
            img_data = requests.get(url, timeout=60).content
            # FORÇA o aspect ratio escolhido pelo user (Grok ignora aspect_ratio quando há foto de input)
            img_data = _force_aspect_ratio(img_data, aspect_ratio)
...
def _padrao_categories_markup(lang):
    cat_texts = {
        "pt": [("👨 Para Homens", "padcat_men"), ("👩 Para Mulheres", "padcat_women"),
               ...
               ("📐 Posters Pro (44 templates)", "padcat_postersmkt"),
        ...
        "es": [("👨 Para Hombres", "padcat_men"), ("👩 Para Mujeres", "padcat_women"),
               ...
               ("📱 Posts Instagram", "padcat_stories"), ("🔥 Sensual", "padcat_sensual"),
               ("📐 Posters Pro (44 plantillas)", "padcat_postersmkt"),
...
def callback_padcat(call):
    ...
    # Atalho: Posters Pro DENTRO do modo Padrao -> usa Grok (Replicate), nao gpt-image-1.
    # Mostra as 5 categorias com prefixo padpost_cat_ para ficar isolado do flow gpt-image-1.
    if cat == "postersmkt":
        bot.answer_callback_query(call.id)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
            ...
            pass
        _show_padrao_posters_categories(call.message.chat.id, user_id, lang)
        return
...
def callback_padst(call):
    ...
    FIX: garante feedback imediato + bloqueia qualquer fallthrough para outros menus (ex: Formato/Settings)."""
    # 1) Resposta imediata ao clique para o Telegram não re-entregar o callback
    try:
        bot.answer_callback_query(call.id)
    except Exception:
        pass
    ...
    if user_id not in pending_photos:
        # Foto perdida/expirou — pede nova foto em vez de cair em qualquer outro menu
        msg = {
            "pt": "📸 A foto expirou. Envia-a novamente e escolhe o estilo outra vez.",
            "en": "📸 Photo expired. Please resend it and pick the style again.",
            "es": "📸 La foto expiró. Envíala de nuevo y elige el estilo otra vez.",
        }.get(lang, "📸 Foto expirada. Envia outra vez.")
        try:
            bot.send_message(call.message.chat.id, msg)
        except Exception:
            pass
    ...
    # NOVO: em vez de gerar direto, mostrar picker de formato (aspect ratio)
    # Mantém a foto em pending_photos (NÃO faz pop) — só fazemos pop após escolher formato
    photo_data = pending_photos[user_id]
    caption = (photo_data.get("caption") or "").strip()
    ...
    # Guarda escolha de estilo para usar após user escolher o formato
    pending_padst_choice[user_id] = {"style_key": style_key, "style_nome": style["nome"]}

    title = {
        "pt": f"📐 <b>Formato da Imagem</b>\n\n🎯 Estilo: <b>{style['nome']}</b>\n\nEscolhe o tamanho:",
        "en": f"📐 <b>Image Format</b>\n\n🎯 Style: <b>{style['nome']}</b>\n\nChoose size:",
        "es": f"📐 <b>Formato de Imagen</b>\n\n🎯 Estilo: <b>{style['nome']}</b>\n\nElige el tamaño:",
    }.get(lang, None) or f"📐 <b>Formato</b>\n🎯 {style['nome']}\n\nEscolhe o tamanho:"

    mk = telebot.types.InlineKeyboardMarkup(row_width=2)
    for k, val in ASPECT_RATIOS.items():
        mk.add(telebot.types.InlineKeyboardButton(
            f"{val['emoji']} {val['desc']}",
            callback_data=f"padfmt_{k}"
        ))
    back_label = {"pt": "⬅️ Voltar aos estilos", "en": "⬅️ Back to styles", "es": "⬅️ Volver a estilos"}.get(lang, "⬅️ Voltar")
    mk.add(telebot.types.InlineKeyboardButton(back_label, callback_data="padflow_styles"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("padfmt_"))
def callback_padfmt(call):
    """Recebe a escolha de formato (aspect ratio) e dispara a geração com Modelo Padrão."""
    try:
        bot.answer_callback_query(call.id)
    except Exception:
        pass
    ...
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    aspect_key = call.data.replace("padfmt_", "")

    choice = pending_padst_choice.pop(user_id, None)
    if not choice or user_id not in pending_photos:
        msg = {
            "pt": "📸 Sessão expirada. Envia a foto novamente.",
            "en": "📸 Session expired. Please resend the photo.",
            "es": "📸 Sesión expirada. Envía la foto de nuevo.",
        }.get(lang, "📸 Sessão expirada.")
        try:
            bot.send_message(call.message.chat.id, msg)
        except Exception:
            pass
    ...
    # Guarda preferência do user para próximas gerações (mantém setting global)
    try:
        set_user_style(user_id, aspect_ratio=aspect_key)
    except Exception:
        pass

    style_key = choice["style_key"]
    style_nome = choice["style_nome"]
    ...
    if not final_prompt:
        bot.send_message(call.message.chat.id, "❌ Erro a montar o prompt.")
        return
    ...
    Thread(target=execute_padrao,
           args=(call.message.chat.id, user_id, lang, photo_data, final_prompt, style_nome, style_key)).start()
...
def handle_all_messages(message):
    ...
    if user_states.get(user_id) == "poster_awaiting_answer":
        try:
            handle_poster_answer(message)
        except Exception as e:
            logger.error(f"poster answer err: {e}")
        return

    # >>> POSTERS PRO: aguardar pedido de edicao em texto livre (loop com vision)
    if user_states.get(user_id) == "poster_awaiting_edit_text":
        try:
            user_states.pop(user_id, None)
            txt = (message.text or "").strip()
            if not txt:
                bot.reply_to(message, "✏️ Escreve o que queres mudar.")
                user_states[user_id] = "poster_awaiting_edit_text"
                return
            Thread(target=_poster_apply_edit,
                    args=(message.chat.id, user_id, get_user_lang(user_id), txt)).start()
        except Exception as e:
            logger.error(f"poster edit txt err: {e}")
        return
...
# ====================================================================
# 🎨 POSTERS PRO — 3º modo (templates fixos com substituicao de placeholders)
# Usa OpenAI gpt-image-1 (chave do utilizador) — 1024x1536 vertical
# 5 categorias: music, food, fitness, motivational, flyers — 44 templates totais
# ====================================================================

POSTER_TEMPLATES_FILE = "poster_templates.json"
POSTER_COST = 15  # creditos por imagem gerada (1a vez)
POSTER_EDIT_COST = 5  # creditos por edicao subsequente (loop com vision)
POSTER_MAX_EDITS = 3  # numero maximo de edicoes por imagem
POSTER_IMG_QUALITY = "high"

# Tamanhos suportados pelo gpt-image-1
POSTER_SIZES = {
    "vertical":  {"value": "1024x1536", "key_label": "size_vertical"},
    "square":    {"value": "1024x1024", "key_label": "size_square"},
    "landscape": {"value": "1536x1024", "key_label": "size_landscape"},
}
POSTER_DEFAULT_SIZE = "vertical"
POSTER_IMG_SIZE = POSTER_SIZES[POSTER_DEFAULT_SIZE]["value"]  # fallback legado

# Estado per-user para o tamanho escolhido (em memoria — persistente o suficiente)
poster_user_size = {}  # user_id -> "vertical" | "square" | "landscape"

def get_user_poster_size_key(user_id):
    return poster_user_size.get(user_id, POSTER_DEFAULT_SIZE)

def get_user_poster_size_value(user_id):
    return POSTER_SIZES[get_user_poster_size_key(user_id)]["value"]

# carregamento lazy para evitar problemas se o ficheiro nao existir
_POSTER_DATA = None
def get_poster_data():
    global _POSTER_DATA
    if _POSTER_DATA is None:
        # 1ª tentativa: ficheiro JSON local (preferido — permite edicao sem mexer em codigo)
        try:
            with open(POSTER_TEMPLATES_FILE, encoding='utf-8') as f:
                _POSTER_DATA = json.load(f)
            logger.info(f"Poster templates loaded from JSON: {sum(len(c['templates']) for c in _POSTER_DATA.values())} total")
            return _POSTER_DATA
        except FileNotFoundError:
            logger.info(f"{POSTER_TEMPLATES_FILE} nao existe — a usar fallback embebido (poster_data.py)")
        except Exception as e:
            logger.error(f"Falha a carregar {POSTER_TEMPLATES_FILE}: {e} — a usar fallback")

        # 2ª tentativa: modulo Python embebido (sempre presente se carregaste poster_data.py)
        try:
            from poster_data import POSTERS_DATA  # type: ignore
            _POSTER_DATA = POSTERS_DATA
            logger.info(f"Poster templates loaded from poster_data.py: {sum(len(c['templates']) for c in _POSTER_DATA.values())} total")
        except Exception as e:
            logger.error(f"FALLBACK FALHOU — Posters Pro vai aparecer vazio. Erro: {e}")
            _POSTER_DATA = {}
    return _POSTER_DATA


def find_poster_template(cat_key, tpl_id):
    data = get_poster_data()
...
# ---------- I18N especifico de Posters Pro ----------
I18N_POSTER = {
    "menu_header": {
        "pt": "🎨 <b>Posters Pro</b>\n\n💳 Créditos: <code>{credits}</code> · {cost}c por poster\n\nEscolhe a categoria:",
        "en": "🎨 <b>Posters Pro</b>\n\n💳 Credits: <code>{credits}</code> · {cost}c per poster\n\nPick a category:",
        "es": "🎨 <b>Posters Pro</b>\n\n💳 Créditos: <code>{credits}</code> · {cost}c por póster\n\nElige la categoría:",
        "fr": "🎨 <b>Posters Pro</b>\n\n💳 Crédits: <code>{credits}</code> · {cost}c par poster\n\nChoisis la catégorie:",
    },
    "category_header": {
        "pt": "{emoji} <b>{name}</b>\n\n{count} templates · escolhe um:",
        "en": "{emoji} <b>{name}</b>\n\n{count} templates · pick one:",
        "es": "{emoji} <b>{name}</b>\n\n{count} plantillas · elige una:",
        "fr": "{emoji} <b>{name}</b>\n\n{count} templates · choisis-en un:",
    },
    "template_header": {
        "pt": "{emoji} <b>{title}</b>{sub}\n\n<i>Categoria: {cat}</i>\n💳 {cost} créditos\n\nComo queres usar este template?",
        "en": "{emoji} <b>{title}</b>{sub}\n\n<i>Category: {cat}</i>\n💳 {cost} credits\n\nHow do you want to use it?",
        "es": "{emoji} <b>{title}</b>{sub}\n\n<i>Categoría: {cat}</i>\n💳 {cost} créditos\n\n¿Cómo quieres usarlo?",
        "fr": "{emoji} <b>{title}</b>{sub}\n\n<i>Catégorie: {cat}</i>\n💳 {cost} crédits\n\nComment veux-tu l'utiliser?",
    },
    "btn_quick": {"pt": "⚡ Rápido (IA decide)", "en": "⚡ Quick (AI decides)", "es": "⚡ Rápido (IA decide)", "fr": "⚡ Rapide (IA décide)"},
    "btn_custom": {"pt": "📝 Personalizar (questionário)", "en": "📝 Customize (questionnaire)", "es": "📝 Personalizar (cuestionario)", "fr": "📝 Personnaliser (questionnaire)"},
    "btn_with_photo": {"pt": "📸 Enviar a minha foto", "en": "📸 Use my photo", "es": "📸 Usar mi foto", "fr": "📸 Utiliser ma photo"},
    "photo_status_yes": {"pt": "✅ Foto recebida", "en": "✅ Photo received", "es": "✅ Foto recibida", "fr": "✅ Photo reçue"},
    "photo_required_warning": {
        "pt": "📸 Esta categoria <b>precisa de uma foto tua / do prato / etc.</b> Envia uma foto neste chat antes de continuar.",
        "en": "📸 This category <b>requires a photo</b> (yours, the dish, etc.). Send a photo in this chat before continuing.",
        "es": "📸 Esta categoría <b>necesita una foto</b> (tuya, del plato, etc.). Envía una en este chat antes de continuar.",
        "fr": "📸 Cette catégorie <b>nécessite une photo</b> (la tienne, du plat, etc.). Envoie-la avant de continuer.",
    },
    "send_photo_now": {
        "pt": "📸 Envia agora a foto que queres usar como referência neste poster.",
        "en": "📸 Now send the photo you want to use as reference for this poster.",
        "es": "📸 Envía ahora la foto que quieres usar como referencia.",
        "fr": "📸 Envoie maintenant la photo de référence pour ce poster.",
    },
    "ask_q_intro": {
        "pt": "📝 <b>Personalizar:</b> vou fazer-te <b>{n}</b> perguntas curtas sobre este template.\nResponde como quiseres ou escreve <code>skip</code> para deixar a IA decidir.",
        "en": "📝 <b>Customize:</b> I will ask <b>{n}</b> short questions about this template.\nAnswer freely or type <code>skip</code> to let the AI decide.",
        "es": "📝 <b>Personalizar:</b> te haré <b>{n}</b> preguntas cortas sobre esta plantilla.\nResponde libremente o escribe <code>skip</code> para que la IA decida.",
        "fr": "📝 <b>Personnaliser:</b> je vais te poser <b>{n}</b> questions courtes sur ce template.\nRéponds librement ou tape <code>skip</code> pour laisser l'IA décider.",
    },
    "ask_q_n": {
        "pt": "❓ <b>Pergunta {i}/{n}</b>\n\n{q}\n\n<i>(escreve <code>skip</code> para deixar a IA decidir)</i>",
        "en": "❓ <b>Question {i}/{n}</b>\n\n{q}\n\n<i>(type <code>skip</code> to let the AI decide)</i>",
        "es": "❓ <b>Pregunta {i}/{n}</b>\n\n{q}\n\n<i>(escribe <code>skip</code> para que la IA decida)</i>",
        "fr": "❓ <b>Question {i}/{n}</b>\n\n{q}\n\n<i>(tape <code>skip</code> pour laisser l'IA décider)</i>",
    },
    "ready_summary": {
        "pt": "✅ <b>Pronto para gerar!</b>\n\n📋 Template: <b>{title}</b>\n📸 Foto: {photo}\n💰 Custo: {cost} créditos\n\nConfirmas?",
        "en": "✅ <b>Ready to generate!</b>\n\n📋 Template: <b>{title}</b>\n📸 Photo: {photo}\n💰 Cost: {cost} credits\n\nConfirm?",
        "es": "✅ <b>Listo para generar!</b>\n\n📋 Plantilla: <b>{title}</b>\n📸 Foto: {photo}\n💰 Coste: {cost} créditos\n\n¿Confirmas?",
        "fr": "✅ <b>Prêt à générer!</b>\n\n📋 Template: <b>{title}</b>\n📸 Photo: {photo}\n💰 Coût: {cost} crédits\n\nConfirmer?",
    },
    "yes": {"pt": "✅ Sim", "en": "✅ Yes", "es": "✅ Sí", "fr": "✅ Oui"},
    "no": {"pt": "❌ Não", "en": "❌ No", "es": "❌ No", "fr": "❌ Non"},
    "no_photo": {"pt": "Sem foto (IA inventa)", "en": "No photo (AI invents)", "es": "Sin foto (IA inventa)", "fr": "Sans photo (IA invente)"},
    "generating": {
        "pt": "🎨 A gerar o teu poster com o <b>Modelo Marketing IA</b>...\nIsto pode levar 30-90 segundos.",
        "en": "🎨 Generating your poster with the <b>Marketing AI Model</b>...\nThis can take 30-90 seconds.",
        "es": "🎨 Generando tu póster con el <b>Modelo Marketing IA</b>...\nPuede tardar 30-90 segundos.",
        "fr": "🎨 Génération de ton poster avec le <b>Modèle Marketing IA</b>...\nCela peut prendre 30-90 secondes.",
    },
    "generated_caption": {
        "pt": "✅ <b>Poster pronto!</b>\n📋 {title}\n💳 Restam: <code>{credits}</code>",
        "en": "✅ <b>Poster ready!</b>\n📋 {title}\n💳 Left: <code>{credits}</code>",
        "es": "✅ <b>Póster listo!</b>\n📋 {title}\n💳 Restan: <code>{credits}</code>",
        "fr": "✅ <b>Poster prêt!</b>\n📋 {title}\n💳 Restants: <code>{credits}</code>",
    },
    "insufficient": {
        "pt": "❌ Créditos insuficientes! Precisas de {need}, tens {have}.",
        "en": "❌ Not enough credits! Need {need}, you have {have}.",
        "es": "❌ Créditos insuficientes! Necesitas {need}, tienes {have}.",
        "fr": "❌ Crédits insuffisants! Il en faut {need}, tu en as {have}.",
    },
    "error_gen": {
        "pt": "❌ Erro a gerar imagem. Créditos reembolsados.\n<code>{err}</code>",
        "en": "❌ Image generation error. Credits refunded.\n<code>{err}</code>",
        "es": "❌ Error al generar la imagen. Créditos reembolsados.\n<code>{err}</code>",
        "fr": "❌ Erreur de génération. Crédits remboursés.\n<code>{err}</code>",
    },
    "menu_btn_label_for_mode": {
        "pt": "🎨 Posters Pro", "en": "🎨 Posters Pro", "es": "🎨 Posters Pro", "fr": "🎨 Posters Pro",
    },
    "mode_set_posters": {
        "pt": "🎨 <b>Modo Posters Pro</b> ativado!\n\nTens acesso a <b>44 templates profissionais</b> em 5 categorias (Música, Comida, Fitness, Motivacional, Flyers). 15 créditos por imagem.",
        "en": "🎨 <b>Posters Pro mode</b> activated!\n\nAccess <b>44 professional templates</b> in 5 categories (Music, Food, Fitness, Motivational, Flyers). 15 credits per image.",
        "es": "🎨 <b>Modo Posters Pro</b> activado!\n\nAcceso a <b>44 plantillas profesionales</b> en 5 categorías. 15 créditos por imagen.",
        "fr": "🎨 <b>Mode Posters Pro</b> activé!\n\nAccès à <b>44 templates professionnels</b> en 5 catégories. 15 crédits par image.",
    },
    # --- TAMANHOS ---
    "size_btn": {"pt": "📐 Tamanho", "en": "📐 Size", "es": "📐 Tamaño", "fr": "📐 Taille"},
    "size_picker_title": {
        "pt": "📐 <b>Escolhe o formato</b>\n\nO tamanho atual é: <b>{current}</b>",
        "en": "📐 <b>Pick the format</b>\n\nCurrent size: <b>{current}</b>",
        "es": "📐 <b>Elige el formato</b>\n\nTamaño actual: <b>{current}</b>",
        "fr": "📐 <b>Choisis le format</b>\n\nFormat actuel: <b>{current}</b>",
    },
    "size_vertical": {
        "pt": "📱 Story / Reel / TikTok (vertical)",
        "en": "📱 Story / Reel / TikTok (vertical)",
        "es": "📱 Story / Reel / TikTok (vertical)",
        "fr": "📱 Story / Reel / TikTok (vertical)",
    },
    "size_square": {
        "pt": "📷 Post Instagram (quadrado)",
        "en": "📷 Instagram Post (square)",
        "es": "📷 Post Instagram (cuadrado)",
        "fr": "📷 Post Instagram (carré)",
    },
    "size_landscape": {
        "pt": "🖼️ Banner / YouTube (horizontal)",
        "en": "🖼️ Banner / YouTube (landscape)",
        "es": "🖼️ Banner / YouTube (horizontal)",
        "fr": "🖼️ Bannière / YouTube (horizontal)",
    },
    "size_set_ok": {
        "pt": "✅ Formato definido: <b>{name}</b>",
        "en": "✅ Format set: <b>{name}</b>",
        "es": "✅ Formato configurado: <b>{name}</b>",
        "fr": "✅ Format défini: <b>{name}</b>",
    },
    # --- LOOP DE EDIÇÃO ---
    "after_gen_q": {
        "pt": "✅ Imagem pronta!\n\nQueres ajustar alguma coisa? (texto, cores, etc.) — <b>cada edição custa 5 créditos</b>",
        "en": "✅ Image ready!\n\nWant to tweak anything? (text, colors, etc.) — <b>each edit costs 5 credits</b>",
        "es": "✅ ¡Imagen lista!\n\n¿Quieres ajustar algo? (texto, colores, etc.) — <b>cada edición cuesta 5 créditos</b>",
        "fr": "✅ Image prête!\n\nTu veux ajuster quelque chose? (texte, couleurs, etc.) — <b>chaque édition coûte 5 crédits</b>",
    },
    "btn_edit_yes": {"pt": "✏️ Sim, mudar algo", "en": "✏️ Yes, change something", "es": "✏️ Sí, cambiar algo", "fr": "✏️ Oui, changer quelque chose"},
    "btn_edit_no": {"pt": "✅ Não, está perfeito", "en": "✅ No, it's perfect", "es": "✅ No, está perfecto", "fr": "✅ Non, c'est parfait"},
    "edit_describe": {
        "pt": "{vision}\n\n✏️ <b>O que queres mudar?</b>\nEscreve livremente, ex:\n• <i>troca o título para 'BEAST MODE'</i>\n• <i>cores azuis em vez de pretas</i>\n• <i>remove o subtítulo</i>",
        "en": "{vision}\n\n✏️ <b>What do you want to change?</b>\nWrite freely, e.g.:\n• <i>change the title to 'BEAST MODE'</i>\n• <i>blue colors instead of black</i>\n• <i>remove the subtitle</i>",
        "es": "{vision}\n\n✏️ <b>¿Qué quieres cambiar?</b>\nEscribe libremente, ej:\n• <i>cambia el título a 'BEAST MODE'</i>\n• <i>colores azules en vez de negros</i>\n• <i>quita el subtítulo</i>",
        "fr": "{vision}\n\n✏️ <b>Que veux-tu changer?</b>\nÉcris librement, ex:\n• <i>change le titre pour 'BEAST MODE'</i>\n• <i>couleurs bleues au lieu de noires</i>\n• <i>supprime le sous-titre</i>",
    },
    "edit_max_reached": {
        "pt": "🔒 Atingiste o máximo de 3 edições para esta imagem. Gera uma nova com /menu.",
        "en": "🔒 You reached the 3-edit limit for this image. Generate a new one via /menu.",
        "es": "🔒 Alcanzaste el límite de 3 ediciones. Genera una nueva con /menu.",
        "fr": "🔒 Tu as atteint la limite de 3 éditions. Génère une nouvelle via /menu.",
    },
    "edit_processing": {
        "pt": "✏️ A aplicar a tua edição com {model}... (≈30-60s)",
        "en": "✏️ Applying your edit with {model}... (≈30-60s)",
        "es": "✏️ Aplicando tu edición con {model}... (≈30-60s)",
        "fr": "✏️ Application de l'édition avec {model}... (≈30-60s)",
    },
    "edit_caption": {
        "pt": "✏️ Edição #{n}/3 aplicada\n💳 Restam: <code>{credits}</code>",
        "en": "✏️ Edit #{n}/3 applied\n💳 Left: <code>{credits}</code>",
        "es": "✏️ Edición #{n}/3 aplicada\n💳 Restan: <code>{credits}</code>",
        "fr": "✏️ Édition #{n}/3 appliquée\n💳 Restants: <code>{credits}</code>",
    },
    "edit_done": {
        "pt": "👍 Sem problema, a imagem fica como está. Usa /menu para criar outra.",
        "en": "👍 No problem, the image stays as is. Use /menu to create another.",
        "es": "👍 No hay problema, la imagen queda así. Usa /menu para crear otra.",
        "fr": "👍 Pas de souci, l'image reste telle quelle. Utilise /menu pour en créer une autre.",
    },
    "vision_intro": {
        "pt": "🤖 <b>O que vejo nesta imagem:</b>\n\n{desc}",
        "en": "🤖 <b>What I see in this image:</b>\n\n{desc}",
        "es": "🤖 <b>Lo que veo en esta imagen:</b>\n\n{desc}",
        "fr": "🤖 <b>Ce que je vois dans cette image:</b>\n\n{desc}",
    },
}


def TP(lang, key, **fmt):
    entry = I18N_POSTER.get(key, {})
    text = entry.get(lang) or entry.get("pt") or key
    if fmt:
        try:
            return text.format(**fmt)
...
# ---------- States ----------
poster_states = {}  # user_id -> {category, template_id, mode (quick/custom), answers, q_idx, questions, photo_bytes}


def _cat_name(cat, lang):
    return cat.get(f"name_{lang}") or cat.get("name_pt", "")


def main_keyboard_posters(user_id, lang="pt"):
    """Menu principal do modo Posters Pro — categorias + utilitarios."""
    data = get_poster_data()
    markup = telebot.types.InlineKeyboardMarkup(row_width=2)
    ...
    for k in cat_keys:
        c = data[k]
        label = f"{c['emoji']} {_cat_name(c, lang)}"
        pairs.append(telebot.types.InlineKeyboardButton(label, callback_data=f"poster_cat_{k}"))
    # 2 por linha
    for i in range(0, len(pairs), 2):
        markup.row(*pairs[i:i+2])
    # Tamanho (mostra o atual no botao)
    cur_key = get_user_poster_size_key(user_id)
    cur_label = TP(lang, POSTER_SIZES[cur_key]["key_label"])
    # Pega so o emoji+nome curto (ate "/")
    short = cur_label.split('/')[0].strip() if '/' in cur_label else cur_label
    markup.add(telebot.types.InlineKeyboardButton(
        f"{TP(lang, 'size_btn')}: {short}",
        callback_data="poster_size_picker"))
    # extras
    markup.add(
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_credits"), callback_data="action_credits"),
        telebot.types.InlineKeyboardButton(T(lang, "fast_btn_buy"), callback_data="action_buy"),
...
def show_posters_main_menu(chat_id, user_id, lang):
    creditos = get_user_credits(user_id)
    data = get_poster_data()
    if not data:
        # Templates nao carregados — diagnostico claro em vez de menu vazio
        diag = (
            "⚠️ <b>Posters Pro indisponivel</b>\n\n"
            "O ficheiro de templates nao foi encontrado no servidor.\n"
            "Falta carregar 1 dos seguintes ficheiros junto com o bot.py:\n"
            "• <code>poster_templates.json</code> (preferido)\n"
            "• <code>poster_data.py</code> (fallback embebido)\n\n"
            "Contacta o admin."
        )
        bot.send_message(chat_id, diag, parse_mode='HTML')
        return

    # Status da foto guardada
    has_photo = bool((poster_states.get(user_id) or {}).get("photo_bytes"))
    photo_line = ""
    if has_photo:
        photo_line = {
            "pt": "📸 <b>Foto guardada</b> ✅ (envia outra para trocar)\n\n",
            "en": "📸 <b>Photo saved</b> ✅ (send another to replace)\n\n",
            "es": "📸 <b>Foto guardada</b> ✅ (envía otra para cambiar)\n\n",
            "fr": "📸 <b>Photo enregistrée</b> ✅ (envoie-en une autre pour changer)\n\n",
        }.get(lang, "")
    else:
        photo_line = {
            "pt": "📸 <b>Envia uma foto</b> antes de escolheres o template.\n\n",
            "en": "📸 <b>Send a photo</b> before picking a template.\n\n",
            "es": "📸 <b>Envía una foto</b> antes de elegir la plantilla.\n\n",
            "fr": "📸 <b>Envoie une photo</b> avant de choisir le template.\n\n",
        }.get(lang, "")

    text = photo_line + TP(lang, "menu_header", credits=creditos, cost=POSTER_COST)
    bot.send_message(chat_id, text,
                      reply_markup=main_keyboard_posters(user_id, lang),
                      parse_mode='HTML')


def is_poster_template_locked(cat_key, tpl_id, user_id):
    """Algumas categorias/templates do Posters Pro sao premium.
    Regra: TODA a categoria 'flyers' e premium. Admins e users que ja compraram
    qualquer pacote tem acesso (mesma logica do has_premium_access)."""
    if cat_key != "flyers":
        return False
    try:
        # admins sempre desbloqueado
        if user_id in ADMIN_IDS:
            return False
        # mesma regra do resto do bot — se ja comprou alguma vez, tem acesso
        if has_premium_access(user_id, "flyers"):
            return False
    except Exception:
        pass
    return True
...
# ============== POSTERS DENTRO DO MODO PADRAO (Grok via Replicate) ==============
# Reusa os mesmos 44 templates mas gera com o MODELO_PADRAO (Grok), igual aos
# 65 estilos existentes. Callbacks isolados com prefixo `padpost_` para nao
# colidir com `poster_*` (que e o flow gpt-image-1).

def _show_padrao_posters_categories(chat_id, user_id, lang):
    data = get_poster_data()
    if not data:
        bot.send_message(chat_id, "⚠️ Templates indisponiveis no servidor.")
        return
    txt = {
        "pt": "📐 <b>Posters Pro · gerados com Modelo Padrão (Grok)</b>\n\nEscolhe a categoria:",
        "en": "📐 <b>Posters Pro · generated with Standard Model (Grok)</b>\n\nPick a category:",
        "es": "📐 <b>Posters Pro · generados con Modelo Estándar (Grok)</b>\n\nElige la categoría:",
        "fr": "📐 <b>Posters Pro · générés avec le Modèle Standard (Grok)</b>\n\nChoisis la catégorie:",
    }.get(lang, "📐 Posters Pro")
    mk = telebot.types.InlineKeyboardMarkup(row_width=2)
    pairs = []
    for k, c in data.items():
        pairs.append(telebot.types.InlineKeyboardButton(
            f"{c['emoji']} {_cat_name(c, lang)}", callback_data=f"padpost_cat_{k}"))
    for i in range(0, len(pairs), 2):
        mk.row(*pairs[i:i+2])
    mk.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="padflow_back"))
    bot.send_message(chat_id, txt, reply_markup=mk, parse_mode='HTML')


def _padrao_posters_templates_kb(cat_key, lang, page=0, per_page=8, user_id=None):
    data = get_poster_data()
    cat = data.get(cat_key)
    if not cat:
        return None
    ...
    page_items = tpls[start:end]
    mk = telebot.types.InlineKeyboardMarkup(row_width=1)
    for t in page_items:
        sub = f" · {t['subtag']}" if t['subtag'] else ""
        locked = user_id and is_poster_template_locked(cat_key, t["id"], user_id)
        prefix = "🔒 " if locked else ""
        label = f"{prefix}{t['title'][:30]}{sub}"
        mk.add(telebot.types.InlineKeyboardButton(label,
                callback_data=f"padpost_tpl_{cat_key}_{t['id']}"))
    nav = []
    if start > 0:
        nav.append(telebot.types.InlineKeyboardButton("◀️", callback_data=f"padpost_pg_{cat_key}_{page-1}"))
    if end < len(tpls):
        nav.append(telebot.types.InlineKeyboardButton("▶️", callback_data=f"padpost_pg_{cat_key}_{page+1}"))
    if nav:
        mk.row(*nav)
    mk.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="padpost_back_main"))
    return mk


@bot.callback_query_handler(func=lambda c: c.data.startswith('padpost_'))
def callback_padpost(call):
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    data = call.data

    if data == "padpost_back_main":
        bot.answer_callback_query(call.id)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
    ...
    if data.startswith("padpost_cat_"):
        cat_key = data.replace("padpost_cat_", "")
        cat = get_poster_data().get(cat_key)
        if not cat:
            bot.answer_callback_query(call.id, "?")
            return
        bot.answer_callback_query(call.id)
        text = TP(lang, "category_header",
                  emoji=cat["emoji"], name=_cat_name(cat, lang), count=len(cat["templates"]))
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                                   reply_markup=_padrao_posters_templates_kb(cat_key, lang, 0, user_id=user_id),
                                   parse_mode='HTML')
        except Exception:
    ...
    if data.startswith("padpost_pg_"):
        rest = data.replace("padpost_pg_", "")
        cat_key, _, page = rest.rpartition("_")
        try:
            page = int(page)
        except ValueError:
            page = 0
        bot.answer_callback_query(call.id)
        kb = _padrao_posters_templates_kb(cat_key, lang, page, user_id=user_id)
        if kb:
            try:
    ...
    if data.startswith("padpost_tpl_"):
        rest = data.replace("padpost_tpl_", "", 1)
        cat_key, _, tpl_id = rest.partition("_")
        cat, tpl = find_poster_template(cat_key, tpl_id)
        if not tpl:
            bot.answer_callback_query(call.id, "?")
            return
        if is_poster_template_locked(cat_key, tpl_id, user_id):
            bot.answer_callback_query(call.id, "🔒")
            mk_buy = telebot.types.InlineKeyboardMarkup(row_width=1)
            mk_buy.add(telebot.types.InlineKeyboardButton(T(lang, "fast_btn_buy"), callback_data="action_buy"))
            mk_buy.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data=f"padpost_cat_{cat_key}"))
            bot.send_message(call.message.chat.id,
                              {"pt": "🔒 <b>Template premium</b> · faz uma compra para desbloquear.",
                               "en": "🔒 <b>Premium template</b> · make a purchase to unlock.",
                               "es": "🔒 <b>Plantilla premium</b> · haz una compra.",
                               "fr": "🔒 <b>Template premium</b> · fais un achat."}.get(lang, "🔒"),
                              reply_markup=mk_buy, parse_mode='HTML')
            return
        bot.answer_callback_query(call.id)
        # NOVO: em vez de gerar direto, mostra picker de formato
        title = {
            "pt": f"📐 <b>Formato do Poster</b>\n\n🎯 Template: <b>{tpl['title']}</b>\n\nEscolhe o tamanho:",
            "en": f"📐 <b>Poster Format</b>\n\n🎯 Template: <b>{tpl['title']}</b>\n\nChoose size:",
            "es": f"📐 <b>Formato del Poster</b>\n\n🎯 Plantilla: <b>{tpl['title']}</b>\n\nElige el tamaño:",
            "fr": f"📐 <b>Format du Poster</b>\n\n🎯 Template: <b>{tpl['title']}</b>\n\nChoisir la taille:",
        }.get(lang, f"📐 Formato\n🎯 {tpl['title']}\n\nEscolhe o tamanho:")

        mk = telebot.types.InlineKeyboardMarkup(row_width=2)
        for ar_key, val in ASPECT_RATIOS.items():
            mk.add(telebot.types.InlineKeyboardButton(
                f"{val['emoji']} {val['desc']}",
                callback_data=f"padpostfmt_{ar_key}_{cat_key}_{tpl_id}"
            ))
        back_label = {"pt": "⬅️ Voltar", "en": "⬅️ Back", "es": "⬅️ Volver", "fr": "⬅️ Retour"}.get(lang, "⬅️ Voltar")
        mk.add(telebot.types.InlineKeyboardButton(back_label, callback_data=f"padpost_cat_{cat_key}"))
        try:
            bot.edit_message_text(title, call.message.chat.id, call.message.message_id,
                                  reply_markup=mk, parse_mode='HTML')
        except Exception:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("padpostfmt_"))
def callback_padpostfmt(call):
    """Recebe escolha de formato para Posters Pro do Modelo Padrão e gera."""
    try:
        bot.answer_callback_query(call.id)
    except Exception:
        pass
    ...
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    rest = call.data.replace("padpostfmt_", "", 1)
    # formato: <aspect_key>_<cat_key>_<tpl_id>
    parts = rest.split("_", 2)
    if len(parts) < 3:
        bot.answer_callback_query(call.id, "Callback inválido.")
        return
    aspect_key, cat_key, tpl_id = parts[0], parts[1], parts[2]
    ...
    if user_id not in pending_photos:
        msg = {
            "pt": "📸 Foto expirada. Envia novamente.",
            "en": "📸 Photo expired. Resend it.",
            "es": "📸 Foto expirada. Envíala de nuevo.",
        }.get(lang, "📸 Foto expirada.")
        bot.send_message(call.message.chat.id, msg)
        return

    # Guarda preferência para próximas gerações
    try:
        set_user_style(user_id, aspect_ratio=aspect_key)
    except Exception:
        pass
    ...
    Thread(target=_padrao_posters_run,
            args=(call.message.chat.id, user_id, lang, cat_key, tpl_id, aspect_key)).start()
    return


def _padrao_posters_run(chat_id, user_id, lang, cat_key, tpl_id, aspect_ratio_key=None):
    """Gera o template usando o MODELO_PADRAO (Grok via Replicate).
    aspect_ratio_key: chave de ASPECT_RATIOS (ex: 'portrait', 'square'); se None usa preferência do user."""
    cat, tpl = find_poster_template(cat_key, tpl_id)
    if not tpl:
        bot.send_message(chat_id, "❌ Template invalido.")
        return
    if user_id not in pending_photos:
        bot.send_message(chat_id, {
            "pt": "📸 Foto expirou. Envia outra e tenta de novo.",
            "en": "📸 Photo expired. Send another and try again.",
            "es": "📸 Foto expiró. Envía otra y reinténtalo.",
            "fr": "📸 Photo expirée. Renvoie-la et réessaie.",
        }.get(lang))
        return

    custo = MODELO_PADRAO["custo"]
    if get_user_credits(user_id) < custo:
        bot.send_message(chat_id, TP(lang, "insufficient", need=custo, have=get_user_credits(user_id)))
        return
    if not use_credit(user_id, custo):
        bot.send_message(chat_id, TP(lang, "insufficient", need=custo, have=get_user_credits(user_id)))
        return

    proc = bot.send_message(chat_id, {
        "pt": f"🎨 A gerar com o Modelo Padrão (Grok)...",
        "en": f"🎨 Generating with the Standard Model (Grok)...",
        "es": f"🎨 Generando con el Modelo Estándar (Grok)...",
        "fr": f"🎨 Génération avec le Modèle Standard (Grok)...",
    }.get(lang, "🎨 ..."))

    try:
        # Foto -> data URL
        photo = pending_photos[user_id]
        finfo = bot.get_file(photo["file_id"])
        photo_bytes = bot.download_file(finfo.file_path)
        import base64 as _b64
        data_url = "data:image/jpeg;base64," + _b64.b64encode(photo_bytes).decode()

        # Resolve aspect_ratio: usa o escolhido pelo user OU 2:3 como default antigo
        if aspect_ratio_key and aspect_ratio_key in ASPECT_RATIOS:
            ratio_str = ASPECT_RATIOS[aspect_ratio_key]["ratio"]
        else:
            ratio_str = "2:3"

        prompt = tpl["prompt"]
        urls = gerar_imagem_padrao(prompt, aspect_ratio=ratio_str, num_outputs=1,
                                    image_input=data_url, user_id=user_id, lang=lang)
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        if not urls:
            raise RuntimeError("sem output")
        creditos_left = get_user_credits(user_id)
        cap = {
            "pt": f"✅ <b>{tpl['title']}</b> pronto\n💳 Restam: <code>{creditos_left}</code>",
            "en": f"✅ <b>{tpl['title']}</b> ready\n💳 Left: <code>{creditos_left}</code>",
            "es": f"✅ <b>{tpl['title']}</b> listo\n💳 Restan: <code>{creditos_left}</code>",
            "fr": f"✅ <b>{tpl['title']}</b> prêt\n💳 Restants: <code>{creditos_left}</code>",
        }.get(lang)
        # Faz crop pós-processamento para forçar o ratio escolhido (Grok ignora ratio quando há foto)
        try:
            img_bytes = requests.get(urls[0], timeout=60).content
            img_bytes = _force_aspect_ratio(img_bytes, ratio_str)
            bot.send_photo(chat_id, img_bytes, caption=cap, parse_mode='HTML')
        except Exception:
            bot.send_photo(chat_id, urls[0], caption=cap, parse_mode='HTML')
        update_user_stats(user_id, "total_creations")
    except Exception as e:
        add_credits(user_id, custo, "reembolso_padpost")
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        logger.error(f"padrao posters err: {e}")
        bot.send_message(chat_id, _humanize_openai_error(e, lang))


def category_templates_kb(cat_key, lang, page=0, per_page=8, user_id=None):
    data = get_poster_data()
    cat = data.get(cat_key)
    if not cat:
        return None
    ...
    for t in page_items:
        ...
        label = f"{prefix}{t['title'][:30]}{sub}"
        markup.add(telebot.types.InlineKeyboardButton(
            label, callback_data=f"poster_tpl_{cat_key}_{t['id']}"))
    nav = []
    if start > 0:
        nav.append(telebot.types.InlineKeyboardButton("◀️", callback_data=f"poster_pg_{cat_key}_{page-1}"))
    if end < len(tpls):
        nav.append(telebot.types.InlineKeyboardButton("▶️", callback_data=f"poster_pg_{cat_key}_{page+1}"))
    if nav:
        markup.row(*nav)
    markup.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="poster_back_main"))
    return markup


def template_options_kb(cat_key, tpl_id, has_photo, lang):
    markup = telebot.types.InlineKeyboardMarkup(row_width=1)
    photo_label = TP(lang, "btn_with_photo")
    if has_photo:
        photo_label = TP(lang, "photo_status_yes") + " · " + photo_label
    markup.add(
        telebot.types.InlineKeyboardButton(TP(lang, "btn_quick"), callback_data=f"poster_run_{cat_key}_{tpl_id}_quick"),
        telebot.types.InlineKeyboardButton(TP(lang, "btn_custom"), callback_data=f"poster_run_{cat_key}_{tpl_id}_custom"),
        telebot.types.InlineKeyboardButton(photo_label, callback_data=f"poster_photo_{cat_key}_{tpl_id}"),
    )
    markup.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data=f"poster_cat_{cat_key}"))
    return markup


# ---------- AI Helpers (questionnaire + auto fill) ----------
def ai_extract_questions(template, lang):
    """Pede ao GPT-4o-mini para olhar para o prompt e gerar 3-5 perguntas
    no idioma do user, focadas APENAS em campos que o user iria querer mudar
    (titulos, nomes, contactos, frases de chamada). Retorna lista de dicts:
    [{"key": "title", "question": "Que titulo grande queres no poster?"}, ...]
    """
    placeholders = template.get("placeholders", [])
    sys_p = (
        "You are a designer's assistant. Given a poster prompt template and its visible "
        "placeholder strings, produce 3 to 5 SHORT questions in the requested language to ask the user. "
        "ONLY ask about EDITABLE TEXTUAL fields (titles, restaurant names, slogans, contact info, dates, "
        "artist names, call-to-action). DO NOT ask about layout, colors, lighting or design choices — "
        "those stay fixed. Return STRICT JSON: "
        '{"questions": [{"key":"snake_case_id", "question":"...", "replaces": ["EXACT placeholder string from list"]}, ...]} '
        "where 'replaces' is the subset of placeholder strings (verbatim, including punctuation) that the answer "
        "should substitute. If no suitable placeholder, omit 'replaces'."
    )
    lang_names = {"pt": "Portuguese", "en": "English", "es": "Spanish", "fr": "French"}
    user_p = (
        f"Language for questions: {lang_names.get(lang, 'Portuguese')}\n\n"
        f"Placeholders found in template:\n{json.dumps(placeholders, ensure_ascii=False)}\n\n"
        f"Template prompt:\n{template['prompt'][:1800]}"
    )
    try:
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
        ...
        for q in qs[:5]:
            if not isinstance(q, dict):
                continue
            qq = q.get("question", "").strip()
            if not qq:
                continue
            clean.append({
                "key": q.get("key") or f"q{len(clean)+1}",
                "question": qq[:200],
                "replaces": [r for r in (q.get("replaces") or []) if isinstance(r, str)],
            })
        if not clean:
            # fallback: 1 pergunta generica por placeholder (max 3)
            for ph in placeholders[:3]:
                clean.append({
                    "key": slug_safe(ph),
                    "question": f'Texto a usar em vez de "{ph}"?',
                    "replaces": [ph],
                })
        return clean
    except Exception as e:
        logger.warning(f"ai_extract_questions err: {e}")
        # fallback simples
        out = []
        for ph in placeholders[:3]:
            out.append({"key": slug_safe(ph), "question": f'Texto a usar em vez de "{ph}"?', "replaces": [ph]})
        return out


def slug_safe(s):
    s = re.sub(r'[^a-zA-Z0-9]+', '_', s).strip('_').lower()
    return s[:24] or "field"


def ai_apply_answers(template, answers_map, lang):
    """Pega no prompt original, substitui os placeholders pelas respostas do user
    (1:1 quando possivel), e pede a IA para tornar coerente. Retorna prompt final EN.
    """
    base = template["prompt"]

    # Substituicao directa primeiro (por placeholder)
    for ans in answers_map.values():
        for repl in ans.get("replaces", []):
            if repl and ans.get("value"):
                base = base.replace(f'"{repl}"', f'"{ans["value"]}"')
                base = base.replace(repl, ans["value"])

    # Recolher fields que pediram skip (IA decide)
    skipped = [a for a in answers_map.values() if a.get("skipped")]
    if not skipped:
        return base

    # Pedir a IA para preencher os campos em falta de forma coerente, em ingles
    sys_p = (
        "You are a designer assistant editing a fixed poster prompt template. "
        "Some placeholders need to be filled coherently (the user said 'let AI decide'). "
        "RULES: Keep ALL layout/design/composition/lighting words EXACT. Only replace the textual placeholders. "
        "Keep the language of typography labels in the language of the original placeholder (do not translate stylistic labels). "
        "Return ONLY the final updated prompt — no explanation."
    )
    skip_info = "\n".join([f"- {a.get('question','')}: pick something appropriate" for a in skipped])
    user_p = (
        f"Original template (already partially filled):\n{base}\n\n"
        f"Fields the user skipped (you decide):\n{skip_info}"
    )
    try:
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
        ...
        logger.warning(f"ai_apply_answers err: {e}")
        return base


# ==================== SAFETY REWRITER + NSFW HANDLING ====================
# Aplica-se a TODOS os prompts antes de ir para modelos de imagem.
# Protege identidade do sujeito (rosto, corpo, skin tone) e notifica admin
# em caso de tentativa NSFW, com fallback automatico para FLUX.

# Keywords que sugerem NSFW leve/medio (PT/EN/ES/FR)
NSFW_KEYWORDS = [
    "nude", "naked", "topless", "bottomless", "boobs", "breasts", "tits", "nipple", "nipples",
    "sexy", "porn", "hentai", "lingerie", "underwear", "panties",
    "biquini", "biquíni", "bikini", "swimsuit", "bañador", "maillot",
    "fishnet", "thong", "thongs", "garter",
    "nua", "nudez", "pelada", "desnuda", "desnudo",
    "cleavage", "cameltoe", "upskirt",
]
# Compilar uma vez com word-boundary para evitar matches em "Subject", "vibrant", "place", etc.
import re as _re_nsfw
_NSFW_RE = _re_nsfw.compile(
    r'(?<![a-záàâãéêíóôõúç])(' + '|'.join(_re_nsfw.escape(k) for k in NSFW_KEYWORDS) + r')(?![a-záàâãéêíóôõúç])',
    _re_nsfw.IGNORECASE,
)
...
def notify_admin_nsfw(user_id, prompt, model="?", severity="medium"):
    ...
    try:
        txt = (
            ...
            f"📝 Prompt: <code>{(prompt or '')[:500]}</code>\n"
            f"🕒 {datetime.now().strftime('%d/%m %H:%M:%S')}"
        )
        for aid in ADMIN_IDS:
            try:
                bot.send_message(aid, txt, parse_mode='HTML')
            except Exception:
                pass
    except Exception as e:
        logger.warning(f"notify_admin_nsfw err: {e}")


def safety_rewrite_prompt(prompt, lang="pt", has_reference_photo=True, allow_nsfw_lite=False):
    """Reescreve QUALQUER prompt de imagem para:
    - Preservar rosto, identidade, proporcoes corporais, skin tone (se houver foto)
    - Remover instrucoes que mudem anatomia (ex: 'make her thinner', 'bigger breasts')
    - Se allow_nsfw_lite=False: tambem remover pedidos NSFW explicitos
    - Se True: permitir NSFW LEVE (biquini, lingerie) mas manter rosto/corpo

    Usa GPT-4o-mini. Se falhar, devolve o prompt original (fail-open por UX).
    """
    if not prompt:
        return prompt
    try:
        rules_common = (
            "You are a prompt safety rewriter for an image edit/generation bot. "
            "Rewrite the user's request into a CLEAN, CONCISE English prompt that an image model can execute. "
            "ABSOLUTE RULES:\n"
            "1. PRESERVE the subject's face, identity, skin tone, body proportions, age, and ethnicity EXACTLY.\n"
            "2. Remove any instruction that changes anatomy, body size, facial structure, age or race.\n"
            "3. Keep the scene/style/clothing/background changes the user asked for.\n"
            "4. Output ONLY the final prompt — no explanation, no markdown, no quotes.\n"
        )
        if allow_nsfw_lite:
            rules = rules_common + (
                "5. ALLOWED: swimwear, bikini, lingerie, nightwear — but NEVER nudity, genitals, or explicit sexual acts.\n"
                "6. If the user explicitly asks for nudity or sexual content, return the literal string: BLOCKED_NSFW_HARD\n"
            )
        else:
            rules = rules_common + (
                "5. BLOCKED: nudity, underwear, lingerie, bikini, sexual content — strip those from the rewrite and replace with neutral clothing.\n"
            )
        if has_reference_photo:
            rules += "6. Keep the pose and camera angle consistent with the reference photo unless the user asks otherwise.\n"

        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": rules},
                {"role": "user", "content": f"User request (language={lang}):\n{prompt[:1500]}"},
            ],
            temperature=0.2,
            max_tokens=400,
        )
        out = resp.choices[0].message.content.strip().strip('"').strip("'")
        if out == "BLOCKED_NSFW_HARD":
            return None  # caller trata como bloqueio
        return out or prompt
    except Exception as e:
        logger.warning(f"safety_rewrite err: {e}")
        return prompt  # fail-open


def _data_url_to_bytes(image_input):
    """Converte data URL ('data:image/...;base64,...') para bytes. Retorna None se falhar."""
    try:
        if not image_input or not isinstance(image_input, str):
            return None
        if image_input.startswith("data:") and "," in image_input:
            import base64 as _b64
            return _b64.b64decode(image_input.split(",", 1)[1])
        return None
    except Exception:
        return None


def replicate_flux_edit(prompt, photo_bytes):
    """Fallback: usa Replicate FLUX (Modelo Pro) para gerar/editar quando NSFW ou gpt-image-1 falha.
    Retorna bytes PNG. Usa MESMA assinatura que gerar_imagem_pro (param 'images' + data URL),
    senao a foto e ignorada e o modelo inventa uma pessoa nova."""
    import replicate, requests as _req, base64 as _b64
    if not photo_bytes:
        raise RuntimeError("FLUX edit requer foto de referencia.")
    data_url = "data:image/jpeg;base64," + _b64.b64encode(photo_bytes).decode()
    identity_anchor = (
        "Edit the EXACT person from the reference image. "
        "Keep face, hair, skin tone, age, body proportions and identity 100% identical. "
    ...
    final_prompt = identity_anchor + (prompt or "")
    inp = {
    ...
    try:
        output = replicate.run(MODELO_PRO["replicate_id"], input=inp)
        url = None
        if isinstance(output, list) and output:
            url = output[0]
        elif hasattr(output, "url"):
            ...
            url = output
        if not url:
            raise RuntimeError("Flux sem output")
        url = str(url)
        r = _req.get(url, timeout=60)
        r.raise_for_status()
        return r.content
    except Exception as e:
        logger.error(f"replicate_flux_edit err: {e}")
        raise



def model_display_name(lang):
    return {
        "pt": "Modelo Marketing IA",
        "en": "Marketing AI Model",
        "es": "Modelo Marketing IA",
        "fr": "Modèle Marketing IA",
    }.get(lang, "Modelo Marketing IA")


def _humanize_openai_error(e, lang):
    """Converte erros tecnicos do OpenAI em mensagem clara para o utilizador."""
    msg = str(e).lower()
    if "timeout" in msg or "timed out" in msg or "connection aborted" in msg:
        return {
            "pt": "⏱️ O modelo demorou demasiado a responder. Tenta de novo — costuma funcionar à 2ª.",
            "en": "⏱️ The model took too long to reply. Try again — usually works on the 2nd attempt.",
            "es": "⏱️ El modelo tardó demasiado. Intenta de nuevo — suele funcionar al 2º intento.",
            "fr": "⏱️ Le modèle a mis trop de temps. Réessaie — ça marche souvent au 2ème essai.",
        }.get(lang, "⏱️ Timeout. Tenta novamente.")
    if "rate" in msg and "limit" in msg:
        return {
            "pt": "🚦 Limite de pedidos da OpenAI atingido. Espera 30s e tenta de novo.",
            "en": "🚦 OpenAI rate limit reached. Wait 30s and try again.",
            "es": "🚦 Límite de OpenAI alcanzado. Espera 30s e intenta de nuevo.",
            "fr": "🚦 Limite OpenAI atteinte. Attends 30s et réessaie.",
        }.get(lang, "🚦 Rate limit. Aguarda.")
    if "billing" in msg or "quota" in msg or "insufficient_quota" in msg:
        return {
            "pt": "💳 Créditos OpenAI esgotados. Recarrega em platform.openai.com.",
            "en": "💳 OpenAI credits exhausted. Top up at platform.openai.com.",
            "es": "💳 Créditos OpenAI agotados. Recarga en platform.openai.com.",
            "fr": "💳 Crédits OpenAI épuisés. Recharge sur platform.openai.com.",
        }.get(lang, "💳 OpenAI sem creditos.")
    if "content_policy" in msg or "safety" in msg or "moderation" in msg:
        return {
            "pt": "🛡️ A imagem foi bloqueada pela política de conteúdo. Tenta uma foto/prompt diferente.",
            "en": "🛡️ The image was blocked by content policy. Try a different photo/prompt.",
            "es": "🛡️ La imagen fue bloqueada por la política de contenido. Prueba con otra foto.",
            "fr": "🛡️ Image bloquée par la politique de contenu. Essaie une autre photo.",
        }.get(lang, "🛡️ Conteudo bloqueado.")
    return {
        "pt": "❌ Erro a gerar a imagem. Tenta de novo daqui a uns segundos.",
        "en": "❌ Image generation failed. Try again in a few seconds.",
        "es": "❌ Error al generar la imagen. Intenta de nuevo en unos segundos.",
        "fr": "❌ Échec de génération. Réessaie dans quelques secondes.",
    }.get(lang, "❌ Erro a gerar.")


def openai_generate_poster(prompt, photo_bytes=None, size=None, _attempt=1, user_id=None, lang="pt", allow_nsfw_lite=False):
    """Gera imagem com OpenAI gpt-image-1. Com retry automatico (1 tentativa extra) em caso de timeout/connection.
    Se o prompt violar content policy do gpt-image-1 -> fallback automatico para Replicate FLUX (silencioso).

    Se photo_bytes for fornecido -> usa images.edit
    Senao -> images.generate
    Retorna bytes da imagem PNG.
    """
    import base64 as _b64

    # SAFETY REWRITE sempre aplicado (preserva identidade)
    rewritten = safety_rewrite_prompt(prompt, lang=lang,
                                       has_reference_photo=bool(photo_bytes),
                                       allow_nsfw_lite=allow_nsfw_lite)
    if rewritten is None:  # bloqueio duro
        raise RuntimeError("Pedido bloqueado: conteudo explicito.")
    if rewritten and rewritten != prompt:
        logger.info(f"Prompt safety-rewritten (user={user_id})")
    use_prompt = rewritten or prompt

    # NSFW attempt detection + admin notify + AUTO-ROUTE para FLUX se for NSFW e tiver foto
    nsfw_flag = is_nsfw_attempt(prompt) or (allow_nsfw_lite and is_nsfw_attempt(use_prompt))
    if nsfw_flag and photo_bytes:
        # Skip gpt-image-1 (que rejeita) e vai direto para FLUX
        if user_id:
            notify_admin_nsfw(user_id, prompt, model="gpt-image-1->flux(auto)", severity="auto-route")
        try:
            return replicate_flux_edit(use_prompt, photo_bytes)
        except Exception as e:
            logger.error(f"FLUX auto-route falhou: {e}")
            # cai para gpt-image-1 mesmo assim (vai provavelmente falhar mas damos chance)
    if nsfw_flag and user_id:
        notify_admin_nsfw(user_id, prompt, model="gpt-image-1",
                           severity="lite" if allow_nsfw_lite else "attempt")

    use_size = size or POSTER_IMG_SIZE
    try:
        if photo_bytes:
        ...
        msg = str(e).lower()
        # retry uma vez em caso de timeout / connection / 5xx transitorios
        if _attempt < 2 and any(k in msg for k in ("timeout", "timed out", "connection", "503", "502", "504")):
            logger.warning(f"openai_generate_poster retry (1/1) apos: {e}")
            time.sleep(2)
        ...
        # FALLBACK automatico para FLUX quando content_policy/safety
        if any(k in msg for k in ("content_policy", "safety", "moderation", "not allowed", "rejected")):
            logger.info(f"gpt-image-1 bloqueou por policy — fallback para FLUX (user={user_id})")
            if user_id:
                notify_admin_nsfw(user_id, prompt, model="gpt-image-1->flux", severity="fallback")
            try:
                return replicate_flux_edit(use_prompt, photo_bytes)
            except Exception as e2:
                logger.error(f"Flux fallback tambem falhou: {e2}")
                raise
        raise


# ---------- CALLBACKS ----------
@bot.callback_query_handler(func=lambda c: c.data.startswith('poster_'))
def callback_poster(call):
    user_id = call.from_user.id
    lang = get_user_lang(user_id)
    data = call.data

    # Voltar ao menu principal de posters
    if data == "poster_back_main":
        bot.answer_callback_query(call.id)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        show_posters_main_menu(call.message.chat.id, user_id, lang)
        return

    # Categoria
    if data.startswith("poster_cat_"):
        cat_key = data.replace("poster_cat_", "")
        cat = get_poster_data().get(cat_key)
        if not cat:
            bot.answer_callback_query(call.id, "Categoria invalida")
            return
        bot.answer_callback_query(call.id)
        text = TP(lang, "category_header",
                  emoji=cat["emoji"], name=_cat_name(cat, lang), count=len(cat["templates"]))
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                                   reply_markup=category_templates_kb(cat_key, lang, 0, user_id=user_id),
                                   parse_mode='HTML')
        except Exception:
            bot.send_message(call.message.chat.id, text,
                              reply_markup=category_templates_kb(cat_key, lang, 0, user_id=user_id),
    ...
    # Paginacao
    if data.startswith("poster_pg_"):
        rest = data.replace("poster_pg_", "")
        cat_key, _, page = rest.rpartition("_")
        try:
            page = int(page)
        except ValueError:
            page = 0
        bot.answer_callback_query(call.id)
        kb = category_templates_kb(cat_key, lang, page, user_id=user_id)
        if kb:
            try:
                bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=kb)
            except Exception:
    ...
    # Template selecionado — fluxo SIMPLIFICADO (igual ao modo Padrao):
    # se ja tem foto, gera direto; se nao tem e o template requer foto, pede.
    if data.startswith("poster_tpl_"):
        rest = data.replace("poster_tpl_", "", 1)
        cat_key, _, tpl_id = rest.partition("_")
        cat, tpl = find_poster_template(cat_key, tpl_id)
        if not tpl:
            bot.answer_callback_query(call.id, "Template invalido")
            return

        # 🔒 Cadeado: bloquear se template e premium e o user nao tem acesso
        if is_poster_template_locked(cat_key, tpl_id, user_id):
            locked_msg = {
                "pt": "🔒 <b>Template premium</b>\n\nFaz qualquer compra para desbloqueares todos os templates Posters Pro premium.",
                "en": "🔒 <b>Premium template</b>\n\nMake any purchase to unlock all premium Posters Pro templates.",
                "es": "🔒 <b>Plantilla premium</b>\n\nHaz cualquier compra para desbloquear todas las plantillas premium.",
                "fr": "🔒 <b>Template premium</b>\n\nFais un achat pour débloquer tous les templates premium.",
            }.get(lang, "🔒 Premium")
            bot.answer_callback_query(call.id, "🔒")
            mk_buy = telebot.types.InlineKeyboardMarkup(row_width=1)
            mk_buy.add(telebot.types.InlineKeyboardButton(T(lang, "fast_btn_buy"), callback_data="action_buy"))
            mk_buy.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data=f"poster_cat_{cat_key}"))
            bot.send_message(call.message.chat.id, locked_msg, reply_markup=mk_buy, parse_mode='HTML')
            return

        bot.answer_callback_query(call.id)

        # Reset/reuse state mantendo foto se ja existir
        prev = poster_states.get(user_id) or {}
        photo_bytes = prev.get("photo_bytes")
        poster_states[user_id] = {
            "category": cat_key,
            "template_id": tpl_id,
        ...
        # Foto SEMPRE obrigatoria em Posters Pro (mesma logica do modo Padrao)
        if not photo_bytes:
            user_states[user_id] = "poster_awaiting_photo"
            poster_states[user_id]["pending_template"] = (cat_key, tpl_id)
            need_photo = {
                "pt": "📸 <b>Envia primeiro a tua foto</b> — vou usá-la como referência neste template.\nDepois de a receber, gero direto.",
                "en": "📸 <b>Send your photo first</b> — I'll use it as the reference for this template.\nAfter I receive it, I'll generate directly.",
                "es": "📸 <b>Envía primero tu foto</b> — la usaré como referencia para esta plantilla.\nDespués de recibirla, genero directo.",
                "fr": "📸 <b>Envoie d'abord ta photo</b> — je l'utiliserai comme référence pour ce template.\nDès que je la reçois, je génère directement.",
            }.get(lang, "📸 Envia primeiro uma foto.")
            bot.send_message(call.message.chat.id, need_photo, parse_mode='HTML')
            return

        # Gera direto, sem mais botoes / sem questionario
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        Thread(target=_poster_generate_first,
                args=(call.message.chat.id, user_id, lang, cat_key, tpl_id)).start()
        return

    # Picker de tamanho
    if data == "poster_size_picker":
        bot.answer_callback_query(call.id)
        cur_key = get_user_poster_size_key(user_id)
        cur_label = TP(lang, POSTER_SIZES[cur_key]["key_label"])
        text = TP(lang, "size_picker_title", current=cur_label)
        mk = telebot.types.InlineKeyboardMarkup(row_width=1)
        for k, info in POSTER_SIZES.items():
            label = TP(lang, info["key_label"])
            if k == cur_key:
                label = "✅ " + label
            mk.add(telebot.types.InlineKeyboardButton(label, callback_data=f"poster_setsize_{k}"))
        mk.add(telebot.types.InlineKeyboardButton(T(lang, "back"), callback_data="poster_back_main"))
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                                   reply_markup=mk, parse_mode='HTML')
        except Exception:
    ...
    if data.startswith("poster_setsize_"):
        size_key = data.replace("poster_setsize_", "")
        if size_key not in POSTER_SIZES:
            bot.answer_callback_query(call.id, "?")
            return
        poster_user_size[user_id] = size_key
        bot.answer_callback_query(call.id, "✅")
        label = TP(lang, POSTER_SIZES[size_key]["key_label"])
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        bot.send_message(call.message.chat.id, TP(lang, "size_set_ok", name=label), parse_mode='HTML')
        time.sleep(0.3)
        show_posters_main_menu(call.message.chat.id, user_id, lang)
        return

    # Botoes pos-geracao: ✏️ editar / ✅ perfeito
    if data == "poster_edit_yes":
        bot.answer_callback_query(call.id)
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        except Exception:
            pass
        Thread(target=_poster_start_edit_loop,
                args=(call.message.chat.id, user_id, lang)).start()
        return

    if data == "poster_edit_no":
        bot.answer_callback_query(call.id)
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        except Exception:
            pass
        bot.send_message(call.message.chat.id, TP(lang, "edit_done"), parse_mode='HTML')
        return

    # legacy buttons (kept for backward compat — simply gera direto)
    if data.startswith("poster_run_"):
        rest = data.replace("poster_run_", "", 1)
        cat_key, _, after = rest.partition("_")
        tpl_id, _, _mode = after.rpartition("_")
        bot.answer_callback_query(call.id)
        Thread(target=_poster_generate_first,
                args=(call.message.chat.id, user_id, lang, cat_key, tpl_id)).start()
        return

    if data.startswith("poster_photo_"):
        rest = data.replace("poster_photo_", "", 1)
        cat_key, _, tpl_id = rest.partition("_")
        bot.answer_callback_query(call.id)
        prev = poster_states.get(user_id) or {}
        prev.update({"category": cat_key, "template_id": tpl_id,
                     "pending_template": (cat_key, tpl_id),
                     "edits_done": 0, "current_image": None})
        poster_states[user_id] = prev
        user_states[user_id] = "poster_awaiting_photo"
        bot.send_message(call.message.chat.id, TP(lang, "send_photo_now"), parse_mode='HTML')
        return

    # Confirmar gerar (legacy)
    if data == "poster_confirm_gen":
        bot.answer_callback_query(call.id)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        st = poster_states.get(user_id)
        if not st:
            bot.send_message(call.message.chat.id, "❌ Sessão expirada. /menu")
            return
        Thread(target=_poster_generate_first,
                args=(call.message.chat.id, user_id, lang, st["category"], st["template_id"])).start()
        return

    if data == "poster_cancel_gen":
        bot.answer_callback_query(call.id)
        poster_states.pop(user_id, None)
        user_states.pop(user_id, None)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
...
def _poster_ask_next_q(chat_id, user_id, lang):
    st = poster_states.get(user_id)
    if not st:
        return
    qs = st["questions"]
    idx = st["q_idx"]
    if idx >= len(qs):
        _poster_finalize(chat_id, user_id, lang, auto=False)
        return
    q = qs[idx]
    user_states[user_id] = "poster_awaiting_answer"
    bot.send_message(chat_id, TP(lang, "ask_q_n", i=idx+1, n=len(qs), q=q["question"]), parse_mode='HTML')


@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "poster_awaiting_answer", content_types=['text'])
def handle_poster_answer(message):
    user_id = message.from_user.id
    lang = get_user_lang(user_id)
    st = poster_states.get(user_id)
    if not st:
        return
    txt = (message.text or "").strip()
    skipped = txt.lower() in ("skip", "saltar", "sauter", "omitir", "pular")
    qs = st["questions"]
    idx = st["q_idx"]
    if idx < len(qs):
        q = qs[idx]
        st["answers"][q["key"]] = {
            "question": q["question"],
            "value": "" if skipped else txt[:200],
            "replaces": q.get("replaces", []),
            "skipped": skipped,
        }
        st["q_idx"] = idx + 1
    user_states.pop(user_id, None)
    if st["q_idx"] >= len(qs):
        _poster_finalize(message.chat.id, user_id, lang, auto=False)
    else:
        _poster_ask_next_q(message.chat.id, user_id, lang)


@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "poster_awaiting_photo", content_types=['photo'])
def handle_poster_photo(message):
    ...
    try:
        ...
        bot.reply_to(message, f"❌ Erro a descarregar foto: {e}")
        return
    if st is None:
        st = {}
    st["photo_bytes"] = downloaded
    ...
    user_states.pop(user_id, None)
    bot.reply_to(message, TP(lang, "photo_status_yes"))
    # Re-mostra opcoes do template (com foto agora marcada)
    cat_key = st.get("category"); tpl_id = st.get("template_id")
    cat, tpl = find_poster_template(cat_key, tpl_id)
    if not tpl:
        return
    sub = f" · {tpl['subtag']}" if tpl['subtag'] else ""
    text = TP(lang, "template_header",
              emoji=cat["emoji"], title=tpl["title"], sub=sub,
              cat=_cat_name(cat, lang), cost=POSTER_COST)
    bot.send_message(message.chat.id, text,
                      reply_markup=template_options_kb(cat_key, tpl_id, True, lang),
                      parse_mode='HTML')


def _poster_finalize(chat_id, user_id, lang, auto):
    st = poster_states.get(user_id)
    if not st:
        return
    cat, tpl = find_poster_template(st["category"], st["template_id"])
    if not tpl:
        return
    photo_label = TP(lang, "photo_status_yes") if st.get("photo_bytes") else TP(lang, "no_photo")
    text = TP(lang, "ready_summary", title=tpl["title"], photo=photo_label, cost=POSTER_COST)
    mk = telebot.types.InlineKeyboardMarkup(row_width=2)
    mk.add(
        telebot.types.InlineKeyboardButton(TP(lang, "yes"), callback_data="poster_confirm_gen"),
        telebot.types.InlineKeyboardButton(TP(lang, "no"), callback_data="poster_cancel_gen"),
    )
    bot.send_message(chat_id, text, reply_markup=mk, parse_mode='HTML')


def _poster_generate_first(chat_id, user_id, lang, cat_key, tpl_id):
    """1ª geracao (template direto). Sem questionario, sem confirmacoes."""
    cat, tpl = find_poster_template(cat_key, tpl_id)
    if not tpl:
        bot.send_message(chat_id, "❌ Template invalido. /menu")
        return

    creditos = get_user_credits(user_id)
    if creditos < POSTER_COST:
        bot.send_message(chat_id, TP(lang, "insufficient", need=POSTER_COST, have=creditos))
        return
    if not use_credit(user_id, POSTER_COST):
        bot.send_message(chat_id, TP(lang, "insufficient", need=POSTER_COST, have=creditos))
        return

    # Auto-fill placeholders com IA (sem perguntar nada ao user)
    final_prompt = ai_apply_answers(tpl, {"_auto": {"skipped": True, "question": "auto-fill all coherent"}}, lang)
    ...
    proc = bot.send_message(chat_id, TP(lang, "generating"), parse_mode='HTML')
    size_value = get_user_poster_size_value(user_id)
    try:
        img_bytes = openai_generate_poster(final_prompt, photo_bytes=st.get("photo_bytes"), size=size_value,
        ...
        # save history
        try:
            creation_id = add_to_history(user_id, "poster", final_prompt[:300], "")
        except Exception:
            creation_id = None
        caption = TP(lang, "generated_caption", title=tpl["title"], credits=creditos_left)
        sent = bot.send_photo(chat_id, img_bytes, caption=caption, parse_mode='HTML')
        # update history url
        try:
            if sent and sent.photo and creation_id:
                hist = load_json(HISTORY_FILE)
                for h in hist.get(str(user_id), []):
                    if h.get("id") == creation_id:
                        h["url"] = sent.photo[-1].file_id
                        break
                save_json(HISTORY_FILE, hist, HISTORY_LOCK)
        except Exception:
            pass
        update_user_stats(user_id, "total_creations")

        # Guarda imagem gerada para o loop de edicao
        st["current_image"] = img_bytes
        poster_states[user_id] = st

        # Pergunta se quer editar
        mk = telebot.types.InlineKeyboardMarkup(row_width=1)
        mk.add(
            telebot.types.InlineKeyboardButton(TP(lang, "btn_edit_yes"), callback_data="poster_edit_yes"),
            telebot.types.InlineKeyboardButton(TP(lang, "btn_edit_no"), callback_data="poster_edit_no"),
        )
        bot.send_message(chat_id, TP(lang, "after_gen_q"), reply_markup=mk, parse_mode='HTML')
    except Exception as e:
        # reembolso
        add_credits(user_id, POSTER_COST, "reembolso_poster")
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        logger.error(f"Poster gen err: {e}")
        bot.send_message(chat_id, _humanize_openai_error(e, lang))


def _poster_start_edit_loop(chat_id, user_id, lang):
    """User clicou ✏️ Sim, mudar algo. Pede ao gpt-4o-mini para descrever a imagem
    (vision) e depois pede ao user o que mudar."""
    st = poster_states.get(user_id) or {}
    if not st.get("current_image"):
        bot.send_message(chat_id, "❌ Imagem expirou. Gera de novo via /menu.")
        return
    if st.get("edits_done", 0) >= POSTER_MAX_EDITS:
        bot.send_message(chat_id, TP(lang, "edit_max_reached"), parse_mode='HTML')
        return

    # Pede a IA para descrever o que esta na imagem
    proc = bot.send_message(chat_id,
        {"pt": "🤖 A analisar a imagem...", "en": "🤖 Analyzing the image...",
         "es": "🤖 Analizando la imagen...", "fr": "🤖 Analyse de l'image..."}.get(lang, "🤖 ..."))
    desc = _vision_describe(st["current_image"], lang)
    try:
        bot.delete_message(chat_id, proc.message_id)
    except Exception:
        pass

    # Mensagem a pedir o que mudar
    vision_block = TP(lang, "vision_intro", desc=desc)
    bot.send_message(chat_id, TP(lang, "edit_describe", vision=vision_block), parse_mode='HTML')
    user_states[user_id] = "poster_awaiting_edit_text"


def _vision_describe(image_bytes, lang):
    """Usa gpt-4o-mini com vision para descrever a imagem em 2-3 frases curtas
    com os elementos textuais e visuais que o user pode querer mudar."""
    import base64 as _b64
    try:
        b64 = _b64.b64encode(image_bytes).decode()
        sys_p = (
            "You are a designer assistant analyzing a poster image. "
            "Describe in 2-3 short bullet points the EDITABLE elements: "
            "main title text (verbatim), subtitle/caption (verbatim), dominant colors, "
            "subject (person/object), and any logos/contact text. "
            "Be concise and concrete. Reply in the requested language."
        )
        lang_names = {"pt": "Portuguese", "en": "English", "es": "Spanish", "fr": "French"}
        user_p = [
            {"type": "text", "text": f"Describe in {lang_names.get(lang,'Portuguese')}, max 4 bullet lines."},
        ...
        logger.warning(f"vision_describe falhou: {e}")
        return {"pt": "(não consegui ler a imagem em detalhe — descreve livremente o que queres mudar)",
                "en": "(could not read image in detail — describe freely what to change)",
                "es": "(no pude leer la imagen en detalle — describe libremente)",
                "fr": "(je n'ai pas pu lire l'image en détail — décris librement)",}.get(lang, "(...)")


def _poster_apply_edit(chat_id, user_id, lang, user_request_text):
    """User descreveu o que quer mudar -> gera nova versao usando images.edit
    com a imagem actual + prompt traduzido para EN para melhor compreensao do gpt-image-1."""
    st = poster_states.get(user_id) or {}
    cur_img = st.get("current_image")
    if not cur_img:
        bot.send_message(chat_id, "❌ Imagem expirou. Gera de novo via /menu.")
        return
    if st.get("edits_done", 0) >= POSTER_MAX_EDITS:
    ...
    # Cobrar creditos
    if get_user_credits(user_id) < POSTER_EDIT_COST:
        bot.send_message(chat_id, TP(lang, "insufficient", need=POSTER_EDIT_COST, have=get_user_credits(user_id)))
        return
    if not use_credit(user_id, POSTER_EDIT_COST):
        bot.send_message(chat_id, TP(lang, "insufficient", need=POSTER_EDIT_COST, have=get_user_credits(user_id)))
        return

    # Pedir a IA para traduzir/refinar o pedido do user em prompt EN curto
    edit_prompt = _translate_edit_prompt(user_request_text, st.get("current_prompt") or "", lang)

    proc = bot.send_message(chat_id,
        TP(lang, "edit_processing", model=model_display_name(lang)),
        parse_mode='HTML')

    size_value = get_user_poster_size_value(user_id)
    try:
        # images.edit com a imagem actual como base
        new_img = openai_generate_poster(edit_prompt, photo_bytes=cur_img, size=size_value,
                                          user_id=user_id, lang=lang, allow_nsfw_lite=False)
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        st["edits_done"] = st.get("edits_done", 0) + 1
        st["current_image"] = new_img
        poster_states[user_id] = st

        creditos_left = get_user_credits(user_id)
        caption = TP(lang, "edit_caption", n=st["edits_done"], credits=creditos_left)
        sent = bot.send_photo(chat_id, new_img, caption=caption, parse_mode='HTML')
        try:
            add_to_history(user_id, "poster_edit", edit_prompt[:300], sent.photo[-1].file_id if sent and sent.photo else "")
        except Exception:
            pass

        # Se ainda tem edits, pergunta de novo
        if st["edits_done"] < POSTER_MAX_EDITS:
            mk = telebot.types.InlineKeyboardMarkup(row_width=1)
            mk.add(
                telebot.types.InlineKeyboardButton(TP(lang, "btn_edit_yes"), callback_data="poster_edit_yes"),
                telebot.types.InlineKeyboardButton(TP(lang, "btn_edit_no"), callback_data="poster_edit_no"),
        ...
        add_credits(user_id, POSTER_EDIT_COST, "reembolso_poster_edit")
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        logger.error(f"Poster edit err: {e}")
        bot.send_message(chat_id, _humanize_openai_error(e, lang))


def _translate_edit_prompt(user_text, original_prompt, lang):
    """Pega no pedido do user (em qualquer lingua) + prompt original e devolve um
    prompt curto em EN focado APENAS na alteracao pedida, mantendo o resto."""
    sys_p = (
        "You are a poster editing assistant. The user wants to edit an already-generated poster image. "
        "Take the user's edit request and produce a CONCISE editing prompt IN ENGLISH for an image-edit model. "
        "Rules: Preserve the overall layout, composition, and design language. Apply ONLY the requested change "
        "(text replacement, color tweak, removing/adding an element). Keep typography style intact unless the user "
        "explicitly mentions it. Output ONLY the final edit prompt, no preamble."
    )
    user_p = (
        f"Original poster prompt (for context, do not redo it):\n{original_prompt[:1500]}\n\n"
        f"User edit request (in their language):\n{user_text[:500]}"
    )
    try:
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
        ...
        return resp.choices[0].message.content.strip()[:2000]
    except Exception as e:
        logger.warning(f"_translate_edit_prompt err: {e}")
        # fallback: usar o pedido literalmente
        return f"Apply this edit to the image while preserving the rest: {user_text}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ---------- MIGRACAO USERS EVER SEEN (anti-abuso credit reset) ----------
        try:
            migrate_existing_users_to_ever_seen()
            print("[BOOT 3/6] ✅ Migracao ever_seen executada", flush=True)
        except Exception as e:
            print(f"[BOOT 3/6] ⚠️ Migracao ever_seen falhou (continuo): {e}", flush=True)
