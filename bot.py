...
def gerar_imagem_padrao(prompt, aspect_ratio="1:1", num_outputs=1, image_input=None, user_id=None, lang="pt"):
    ...
    try:
        # 1. NSFW detection + admin notify
        if user_id and is_nsfw_attempt(prompt):
            notify_admin_nsfw(user_id, prompt, model="padrao(grok)", severity="attempt")
        # 2. Safety rewrite (sempre)
...
def gerar_imagem_pro(image_input, prompt_override=None, user_id=None, lang="pt", allow_nsfw_lite=False):
    ...
    try:
        ...
        if user_id and is_nsfw_attempt(prompt_raw):
            notify_admin_nsfw(user_id, prompt_raw, model="pro(flux)",
        ...
        if prompt_final is None:
            raise RuntimeError("Pedido bloqueado: conteudo explicito.")
        input_params = {
            "prompt": prompt_final or prompt_raw,
            "images": [image_input],
            "aspect_ratio": "match_input_image",
            "safety_tolerance": 6,
            "disable_safety_checker": True
...
def callback_padcat(call):
    ...
    # Atalho: redirecionar para o flow Posters Pro com a foto que ja existe em pending_photos
    if cat == "postersmkt":
        bot.answer_callback_query(call.id)
        # Move a foto para poster_states e abre o menu Posters Pro
        try:
            ph = pending_photos.get(user_id)
            if ph and ph.get("file_id"):
            ...
            logger.warning(f"padcat_postersmkt photo migrate err: {e}")
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
...
NSFW_KEYWORDS = [
    "nude", "naked", "topless", "bottomless", "boobs", "breasts", "tits", "nipple",
    "sex", "sexy", "porn", "hentai", "lingerie", "underwear", "panties", "bra",
    "biquini", "biquíni", "bikini", "swimsuit", "bañador", "maillot",
    "lace", "lacy", "fishnet", "stocking", "thong", "thongs", "garter",
    "nua", "nudez", "pelada", "sem roupa", "desnuda", "desnudo", "desnuda",
...
def replicate_flux_edit(prompt, photo_bytes):
    """Fallback: usa Replicate FLUX (Modelo Pro) para gerar/editar quando gpt-image-1 falha por policy.
    Retorna bytes PNG."""
    import replicate, requests as _req
    from io import BytesIO
    try:
        # Upload foto primeiro para um URL temporario via telegram ou enviar como file
        inp = {
            "prompt": prompt[:2000],
            "output_format": "png",
            "aspect_ratio": "9:16",
        ...
        if photo_bytes:
            # replicate aceita URL OU file-like
            bio = BytesIO(photo_bytes); bio.name = "ref.png"
            inp["input_image"] = bio
        output = replicate.run(MODELO_PRO["replicate_id"], input=inp)
        # output pode ser URL, iterator, ou FileOutput
        url = None
        if isinstance(output, list) and output:
            url = output[0]
        elif hasattr(output, "url"):
...
def openai_generate_poster(prompt, photo_bytes=None, size=None, _attempt=1, user_id=None, lang="pt", allow_nsfw_lite=False):
    ...
    # NSFW attempt detection + admin notify
