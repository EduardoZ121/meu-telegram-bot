...
POSTER_COST = 15  # creditos por imagem gerada
POSTER_IMG_SIZE = "1024x1536"  # vertical (gpt-image-1)
...
I18N_POSTER = {
    ...
    "generating": {
        "pt": "🎨 A gerar o teu poster com gpt-image-1...\nIsto pode levar 30-60 segundos.",
        "en": "🎨 Generating your poster with gpt-image-1...\nThis can take 30-60 seconds.",
        "es": "🎨 Generando tu póster con gpt-image-1...\nPuede tardar 30-60 segundos.",
        "fr": "🎨 Génération de ton poster avec gpt-image-1...\nCela peut prendre 30-60 secondes.",
    },
    "generated_caption": {
    ...
    "mode_set_posters": {
        "pt": "🎨 <b>Modo Posters Pro</b> ativado!\n\nTens acesso a <b>44 templates profissionais</b> em 5 categorias (Música, Comida, Fitness, Motivacional, Flyers). 15 créditos por imagem com gpt-image-1 (OpenAI).",
        "en": "🎨 <b>Posters Pro mode</b> activated!\n\nAccess <b>44 professional templates</b> in 5 categories (Music, Food, Fitness, Motivational, Flyers). 15 credits per image with gpt-image-1 (OpenAI).",
        "es": "🎨 <b>Modo Posters Pro</b> activado!\n\nAcceso a <b>44 plantillas profesionales</b> en 5 categorías. 15 créditos por imagen con gpt-image-1 (OpenAI).",
        "fr": "🎨 <b>Mode Posters Pro</b> activé!\n\nAccès à <b>44 templates professionnels</b> en 5 catégories. 15 crédits par image avec gpt-image-1 (OpenAI).",
    },
}
...
# ---------- IMAGE GENERATION ----------
def openai_generate_poster(prompt, photo_bytes=None):
    ...
    if photo_bytes:
        # PNG with alpha is required for edits; we send as-is and let the API handle JPG too
...
def callback_poster(call):
    ...
    if data.startswith("poster_tpl_"):
        ...
        text = TP(lang, "template_header",
                  emoji=cat["emoji"], title=tpl["title"], sub=sub,
                  cat=_cat_name(cat, lang), cost=POSTER_COST)
        if cat["photo_required"]:
            text += "\n\n" + TP(lang, "photo_required_warning")
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                                   reply_markup=template_options_kb(cat_key, tpl_id, False, lang),
                                   parse_mode='HTML')
        except Exception:
            bot.send_message(call.message.chat.id, text,
                              reply_markup=template_options_kb(cat_key, tpl_id, False, lang),
    ...
    if data.startswith("poster_photo_"):
        ...
        bot.answer_callback_query(call.id)
        # Garantir state
        if user_id not in poster_states:
            poster_states[user_id] = {"category": cat_key, "template_id": tpl_id, "mode": None,
                                       "answers": {}, "questions": [], "q_idx": 0, "photo_bytes": None}
        user_states[user_id] = "poster_awaiting_photo"
        bot.send_message(call.message.chat.id, TP(lang, "send_photo_now"), parse_mode='HTML')
        return

    # Executar (rapido ou personalizado)
    if data.startswith("poster_run_"):
        rest = data.replace("poster_run_", "", 1)
        # rest = "{cat}_{tpl_id}_{mode}"
        # Note: tpl_id can contain underscores. We split off the last token as mode.
        cat_key, _, after = rest.partition("_")
        tpl_id, _, mode = after.rpartition("_")
        cat, tpl = find_poster_template(cat_key, tpl_id)
        if not tpl or mode not in ("quick", "custom"):
            bot.answer_callback_query(call.id, "Erro template/modo")
            return
        bot.answer_callback_query(call.id)

        st = poster_states.get(user_id) or {}
        st["category"] = cat_key
        st["template_id"] = tpl_id
        st["mode"] = mode
        poster_states[user_id] = st

        # Verificar foto obrigatoria
        if cat["photo_required"] and not st.get("photo_bytes"):
            bot.send_message(call.message.chat.id, TP(lang, "photo_required_warning"), parse_mode='HTML')
            user_states[user_id] = "poster_awaiting_photo"
            return

        if mode == "quick":
            _poster_finalize(call.message.chat.id, user_id, lang, auto=True)
            return
        # custom: gerar perguntas
        bot.send_message(call.message.chat.id,
                          {"pt": "🤖 A preparar perguntas...", "en": "🤖 Preparing questions...",
                           "es": "🤖 Preparando preguntas...", "fr": "🤖 Préparation des questions..."}.get(lang, "..."))
        questions = ai_extract_questions(tpl, lang)
        if not questions:
            _poster_finalize(call.message.chat.id, user_id, lang, auto=True)
            return
        ...
        st["q_idx"] = 0
        st["answers"] = {}
        bot.send_message(call.message.chat.id, TP(lang, "ask_q_intro", n=len(questions)), parse_mode='HTML')
        _poster_ask_next_q(call.message.chat.id, user_id, lang)
        return

    # Confirmar gerar (apos summary)
    if data == "poster_confirm_gen":
        ...
        if not st:
            ...
            return
        cat_key = st["category"]; tpl_id = st["template_id"]
        cat, tpl = find_poster_template(cat_key, tpl_id)
        if not tpl:
            bot.send_message(call.message.chat.id, "❌ Template inválido. /menu")
            return
        # Construir prompt final
        if st.get("mode") == "quick" or not st.get("answers"):
            final_prompt = ai_apply_answers(tpl, {"_auto": {"skipped": True, "question": "auto-fill all"}}, lang)
        else:
            final_prompt = ai_apply_answers(tpl, st["answers"], lang)
        Thread(target=_poster_run_generation,
                args=(call.message.chat.id, user_id, lang, tpl, st.get("photo_bytes"), final_prompt)).start()
        return
...
def _poster_run_generation(chat_id, user_id, lang, tpl, photo_bytes, final_prompt):
    creditos = get_user_credits(user_id)
    ...
    try:
        img_bytes = openai_generate_poster(final_prompt, photo_bytes=photo_bytes)
        try:
            bot.delete_message(chat_id, proc.message_id)
        except Exception:
            pass
        ...
        # Save to history (usa file_id apos enviar — tipo "URL" interna do Telegram)
        try:
            creation_id = add_to_history(user_id, "poster", final_prompt[:300], "")
        except Exception:
            creation_id = None
        if creation_id:
            sent = bot.send_photo(chat_id, img_bytes, caption=caption,
                                   reply_markup=creation_actions_keyboard(creation_id, lang),
                                   parse_mode='HTML')
        else:
            sent = bot.send_photo(chat_id, img_bytes, caption=caption, parse_mode='HTML')
        # Atualizar URL com file_id do Telegram (usavel em historico)
        try:
        ...
        bot.send_message(chat_id, TP(lang, "error_gen", err=err), parse_mode='HTML')
    finally:
        # Mantemos foto no state para regenerar facilmente
        if user_id in poster_states:
            poster_states[user_id]["mode"] = None
            poster_states[user_id]["answers"] = {}
            poster_states[user_id]["questions"] = []
            poster_states[user_id]["q_idx"] = 0
